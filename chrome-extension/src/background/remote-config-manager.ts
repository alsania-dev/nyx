...
export class RemoteConfigManager {
  ...
  async initialize(): Promise<void> {
    try { logger.debug('[RemoteConfigManager] Initializing Remote Config Manager...');
      ...
      this.isInitialized = true; logger.debug('[RemoteConfigManager] Remote Config Manager initialized successfully');
  ...
  async fetchConfig(force = false): Promise<void> {
    try {
      ...
      const now = Date.now(); const minInterval = 3600000; // 1 hour - hardcoded since we can't access remoteConfig.settings
 ...
 logger.debug('[RemoteConfigManager] Fetching remote config...');
      ...
      const allConfigs = firebaseRemoteConfigAPI.getAll(); const remoteConfigCount = Object.values(allConfigs).filter(config => config.source === 'remote').length; const defaultConfigCount = Object.values(allConfigs).filter(config => config.source === 'default').length;
  ...
  async processConfiguration(): Promise<void> {
    try {
      ...
      // Debug log: Show all available configuration logger.debug('[RemoteConfigManager] Processing configuration. Available configs:');
  ...
  private async processFeatureFlags(allConfigs: Record<string, any>): Promise<void> {
    try { const featuresRaw = firebaseRemoteConfigAPI.getValue('features');
  ...
  private async processNotifications(allConfigs: Record<string, any>): Promise<void> {
    try { const notificationsRaw = firebaseRemoteConfigAPI.getValue('active_notifications');
  ...
  private async processAdapterConfigurations(allConfigs: Record<string, any>): Promise<void> {
    try {
      ...
      // First, try to get unified adapter_configs parameter const adapterConfigsRaw = firebaseRemoteConfigAPI.getValue('adapter_configs');
      ...
      // Also check for individual adapter config parameters const individualConfigKeys = Object.keys(allConfigs).filter(key => key.endsWith('_adapter_config'));
        ...
        for (const configKey of individualConfigKeys) {
          try {
            ...
            if (configValue.value) {
              const config = JSON.parse(configValue.value); const adapterName = configKey.replace('_adapter_config', '');
      ...
      if (foundConfigs && Object.keys(adapterConfigs).length > 0) {
        ...
        } else { logger.warn('[RemoteConfigManager] Adapter configs validation failed');
        }
      } else { logger.debug('[RemoteConfigManager] No adapter configurations found');
      }
    } catch (error) {
  ...
  private validateAdapterConfigs(configs: any): boolean { if (!configs || typeof configs !== 'object') { logger.warn('[RemoteConfigManager] Invalid adapter configs structure');
      return false;
    }

    // Validate each adapter config has required structure
    for (const [adapterName, config] of Object.entries(configs)) { if (!config || typeof config !== 'object') {
  ...
  private async processVersionConfigurations(allConfigs: Record<string, any>): Promise<void> {
    try {
      ...
      const currentVersion = chrome.runtime.getManifest().version; const versionConfigKey = `version_config_${currentVersion.replace(/\./g, '_')}`;
      ...
      if (versionConfigString) {
        ...
        if (validateConfigContent(versionConfig)) {
          ...
          await this.broadcastVersionConfig(versionConfig); logger.debug('[RemoteConfigManager] Applied version-specific configuration');
        }
      }
    } catch (error) {
  ...
  private async broadcastFeatureFlags(features: Record<string, any>): Promise<void> {
    try {
      ...
      for (const tab of tabs) {
        if (tab.id) {
          try {
            await chrome.tabs.sendMessage(tab.id, { type: 'remote-config:feature-flags-updated',
              data: {
                flags: features,
                timestamp: Date.now()
              }
            ...
            // Tab may not have content script injected, ignore logger.debug('[RemoteConfigManager] Could not send message to tab:', tab.id);
          }
        }
      }
    } catch (error) {
  ...
  private async broadcastNotifications(notifications: any[]): Promise<void> {
    try {
      ...
      for (const tab of tabs) {
        if (tab.id) {
          try {
            await chrome.tabs.sendMessage(tab.id, { type: 'remote-config:notifications-received',
              data: {
                notifications,
                timestamp: Date.now()
              }
  ...
  private async broadcastAdapterConfigs(adapterConfigs: Record<string, any>): Promise<void> {
    try {
      ...
      for (const tab of tabs) { if (tab.id && tab.url && !tab.url.startsWith('chrome://') && !tab.url.startsWith('chrome-extension://')) {
          try {
            await chrome.tabs.sendMessage(tab.id, { type: 'remote-config:adapter-configs-updated',
              data: {
                adapterConfigs,
                timestamp: Date.now()
              }
  ...
  private async broadcastVersionConfig(versionConfig: any): Promise<void> {
    try {
      ...
      for (const tab of tabs) {
        if (tab.id) {
          try {
            await chrome.tabs.sendMessage(tab.id, { type: 'remote-config:version-config-updated',
              data: {
...
//     }, 5000); //     logger.debug('[RemoteConfigManager] Started periodic config fetching');
//   }
  private startPeriodicFetch(): void {
  ...
  private stopPeriodicFetch(): void {
    if (this.fetchInterval) {
      ...
      this.fetchInterval = null; logger.debug('[RemoteConfigManager] Stopped periodic config fetching');
    }
  }
  ...
  private async getLastFetchTime(): Promise<number | null> { const result = await chrome.storage.local.get(['remoteConfigLastFetch']);
  ...
  async getSpecificConfig(key: string): Promise<any> {
    try {
      ...
      // Handle adapter configs specially if (key.includes('adapter_config')) { const adapterConfigsRaw = firebaseRemoteConfigAPI.getValue('adapter_configs');
        const adapterConfigsString = adapterConfigsRaw.value;
        
        if (adapterConfigsString) {
          const adapterConfigs = JSON.parse(adapterConfigsString);
          
          // Extract specific adapter config if (key.endsWith('_adapter_config')) { const adapterName = key.replace('_adapter_config', '');
  ...
  async cleanup(): Promise<void> {
    ...
    this.retryCount = 0; logger.debug('[RemoteConfigManager] Cleaned up');
  }
  ...
  async clearCacheAndRefresh(): Promise<boolean> {
    try { logger.debug('[RemoteConfigManager] Clearing cache and forcing refresh...');
      ...
      if (success) {
        ...
        await this.processConfiguration(); logger.debug('[RemoteConfigManager] Cache cleared and config refreshed successfully');
      }
