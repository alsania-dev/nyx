...
import {
  ...
  createMcpClient,
  type TransportType, type ConnectionRequest
} from '../mcpclient/index';
import { sendAnalyticsEvent, trackError, collectDemographicData
...
import type {
  ...
  ToolUpdateBroadcast,
  ServerConfigUpdatedBroadcast, HeartbeatResponseBroadcast
} from '../../../pages/content/src/types/messages';
import { createLogger
} from '@extension/shared/lib/logger';

// Default MCP server URLs const logger = createLogger('BACKGROUND'); const DEFAULT_SSE_URL = 'http: //localhost:3055/sse';
...
async function initializeServerConfig(): Promise<void> {
  try { const result = await chrome.storage.local.get(['mcpServerUrl', 'mcpConnectionType'
    ]);
    
    // Load connection type first to determine default URL
    connectionType = (result.mcpConnectionType as ConnectionType) || DEFAULT_CONNECTION_TYPE; const defaultUrl = connectionType === 'websocket' 
      ? DEFAULT_WEBSOCKET_URL  : connectionType === 'streamable-http'
...
function updateServerConfig(url: string, type?: ConnectionType): void {
  ...
  if (type) {
    connectionType = type;
  } logger.debug('[Background] Server config updated to:',
  { url, type: connectionType
  });
}
/**
...
function updateConnectionStatus(status: boolean): void {
  isConnected = status; logger.debug('[Background] Connection status updated to:', status);
}
/**
...
function categorizeToolError(error: Error): { isConnectionError: boolean; isToolError: boolean; category: string
 ...
 // Tool-specific errors that definitely don't indicate connection issues
  ...
  // Default to tool error for ambiguous cases to prevent unnecessary disconnections return { isConnectionError: false, isToolError: true, category: 'unknown_tool_error'
  };
}
/**
 ...
 */
async function initializeExtension() { sendAnalyticsEvent('extension_loaded',
  {}); logger.debug('Extension initializing...');

  // Initialize theme
  try {
    const theme = await exampleThemeStorage.get(); logger.debug('Theme initialized:', theme);
  } catch (error) {
  ...
  const serverUrl = getServerUrl(); logger.debug('Background script initialized with server URL:', serverUrl);
 ...
 logger.debug('Extension initialized successfully');
  ...
  const checkInitialConnectionStatus = async () => {
    ...
    logger.debug(`Attempting initial connection to ${serverUrl
    } with transport: ${connectionType
    }`);
    
    // Try to connect to the server immediately
    let isConnected = false;
    try {
      await tryConnectToServer(serverUrl, connectionType); // After connection attempt, check if we're actually connected
    ...
    if (isConnected) {
      try { logger.debug('[Background] Server connected, fetching and broadcasting initial tools...');
        const primitives = await getPrimitivesWithBackwardsCompatibility(serverUrl,
        false, connectionType);
        logger.debug(`Retrieved ${primitives.length
        } primitives for initial broadcast`);
        
        const tools = normalizeTools(primitives);
        logger.debug(`Broadcasting ${tools.length
        } normalized initial tools`);
...
async function tryConnectToServer(uri: string, type: ConnectionType = connectionType): Promise<void> {
  if (isConnecting) { logger.debug('Connection attempt already in progress, skipping');
    return;
  }
  ...
  logger.debug(
    `Attempting to connect to MCP server via ${type
 ...
 logger.debug('MCP client connected successfully');
    ...
    try { logger.debug('[Background] Connection successful, fetching and broadcasting tools...');
      const primitives = await getPrimitivesWithBackwardsCompatibility(uri,
      true, type);
      logger.debug(`Retrieved ${primitives.length
      } primitives after connection`);
      
      const tools = normalizeTools(primitives);
      logger.debug(`Broadcasting ${tools.length
      } normalized tools after successful connection`);
    ...
    logger.warn(`MCP server connection failed (${errorCategory.category
    ...
    } else { logger.debug('Error categorized as tool-related, not updating connection status');
    } // Schedule another attempt if we haven't reached the limit
    if (connectionAttemptCount < MAX_CONNECTION_ATTEMPTS) {
      const delayMs = Math.min(5000 * connectionAttemptCount,
      15000); // Exponential backoff with cap
      logger.debug(`Scheduling next connection attempt in ${delayMs / 1000
      } seconds...`);
      ...
      logger.debug('Maximum connection attempts reached. Will try again during periodic check.'); // ENHANCED: Don't give up permanently - periodic checks will retry with reset state
      isConnecting = false;
    }
  } finally {
    if (connectionAttemptCount >= MAX_CONNECTION_ATTEMPTS) {
...
setInterval(async () => {
  ...
  if (wasConnected !== isConnected) {
    logger.debug(`Connection status changed: ${wasConnected
    } -> ${isConnected
    }`);
    broadcastConnectionStatusToContentScripts(isConnected);
    ...
    if (isConnected) {
      try {
        ...
        const primitives = await getPrimitivesWithBackwardsCompatibility(getServerUrl(),
        true, connectionType);
        logger.debug(`Periodic check: Retrieved ${primitives.length
        } primitives`);
        
        const tools = normalizeTools(primitives);
        logger.debug(`Periodic check: Broadcasting ${tools.length
        } normalized tools`);
  ...
  } else { // Even if status didn't change, periodically broadcast to ensure content scripts are in sync
    broadcastConnectionStatusToContentScripts(isConnected);
  }
  ...
  if (!isConnected && !isConnecting) {
    connectionAttemptCount = 0; // Reset counter for periodic checks logger.debug('Periodic check: MCP server not connected, attempting to connect');
    const serverUrl = getServerUrl();
...
self.addEventListener('unhandledrejection', event => { logger.error('Unhandled rejection in service worker:', event.reason);
  if (event.reason instanceof Error) { trackError(event.reason, 'background_unhandled_rejection');
  } else {
    // Handle non-Error rejections if necessary sendAnalyticsEvent('extension_error',
    {
      error_message: `Unhandled rejection: ${JSON.stringify(event.reason)
      }`, error_context: 'background_unhandled_rejection_non_error',
    });
  }
}); self.addEventListener('error', event => { logger.error('Uncaught error in service worker:', event.error);
  if (event.error instanceof Error) { trackError(event.error, 'background_uncaught_error');
  } else { sendAnalyticsEvent('extension_error',
    {
...
// --- Lifecycle Events ---
  ...
  chrome.runtime.onInstalled.addListener(async details => { logger.debug('Extension installed or updated:', details.reason);

  const currentVersion = chrome.runtime.getManifest().version;
  const installDate = new Date().toISOString();

  // Perform initial setup on first install if (details.reason === 'install') { logger.debug('Performing first-time installation setup.');
    ...
    // Track installation with enhanced data sendAnalyticsEvent('extension_installed',
    {
    ...
    } } else if (details.reason === 'update') { const previousVersion = details.previousVersion || 'unknown';
    logger.debug(`Extension updated from ${previousVersion
    } to ${currentVersion
    }`);
    ...
    // Track update with enhanced data sendAnalyticsEvent('extension_installed',
    {
      reason: details.reason,
      extension_version: currentVersion,
      previous_version: previousVersion,
...
chrome.runtime.onStartup.addListener(() => {
  logger.debug('Browser startup detected.'); sendAnalyticsEvent('browser_startup',
  {});
  // Re-check connection on startup initializeExtension().catch(err => logger.error('Error initializing on startup:', err));
});

// Start extension initialization
initializeExtension()
  .then(() => { logger.debug('Extension startup complete');
})
  .catch(error => { logger.error('Error during extension initialization:', error); logger.debug('Extension will continue running with limited functionality');
...
// --- Enhanced Message Handling ---
  ...
  // Enhanced logging for debugging logger.debug('[Background] Received message:',
  {
    type: message.type || message.command, origin: message.origin || 'unknown',
    id: message.id,
    hasPayload: !!message.payload,
    from: sender.tab ? `tab-${sender.tab.id }` : 'extension'
  });

  // Handle MCP client connection status changes if (message.type === 'mcp:connection-status-changed' && message.origin === 'mcpclient') { logger.debug('[Background] Received connection status change from MCP client:', message.payload);
  ...
  /* ------------------------------------------------------------------ */ if (message.command === 'trackAnalyticsEvent') {
    if (message.eventName && message.eventParams) {
      sendAnalyticsEvent(message.eventName, message.eventParams)
        ...
        .catch(error => { logger.error('[Background] Error tracking analytics event from message:', error);
          sendResponse({ success: false, error: error instanceof Error ? error.message : String(error)
        });
      });
      return true; // Async response
    } logger.warn('[Background] Invalid trackAnalyticsEvent message received:', message); sendResponse({ success: false, error: 'Invalid eventName or eventParams'
    });
    return false;
  }
  /* ------------------------------------------------------------------ */
  ...
  // Fallback – message not handled here logger.debug('[Background] Message not handled, ignoring:', message.type || message.command);
  return false;
});
...
async function handleMcpMessage(
  ...
  try {
    ...
    switch (messageType) { case 'mcp:call-tool': {
        const { toolName, args, adapterName
        } = payload as CallToolRequest & { adapterName?: string
        };
 ...
 case 'mcp:get-connection-status': { logger.debug('[Background] Getting current connection status');
        ...
        logger.debug(`Stored status: ${storedStatus
        ...
        result = {  status: actualStatus ? 'connected' : 'disconnected',
 ...
 case 'mcp:get-tools': {
        ...
        try {
          ...
          logger.debug(`Retrieved ${primitives.length
          } primitives from server`);
          
          // Use the helper function to normalize tools with proper schema handling
          const tools = normalizeTools(primitives);
          logger.debug(`Returning ${tools.length
          } normalized tools to content script`);
 ...
 case 'mcp:force-reconnect': { logger.debug('[Background] Force reconnect requested via context bridge');
        
        try {
          // Broadcast reconnection started status broadcastConnectionStatusToContentScripts(false, 'Reconnecting...');
           logger.debug('[Background] Starting force reconnection process...');
          
          // ENHANCED: Reset connection state before attempting reconnection // This ensures we don't get blocked by consecutive failure limits
          resetMcpConnectionState();
          ...
          const timeoutPromise = new Promise<void>((_, reject) => 
            setTimeout(() => reject(new Error('Reconnection timeout after 20 seconds')),
          20000)
          );
          ...
          if (isConnected) {
            try { logger.debug('[Background] Fetching tools after successful reconnection...');
              const primitives = await getPrimitivesWithBackwardsCompatibility(getServerUrl(),
              true, connectionType);
              logger.debug(`Retrieved ${primitives.length
              } primitives after reconnection`);
              
              const tools = normalizeTools(primitives);
              logger.debug(`Broadcasting ${tools.length
              } normalized tools after reconnection`);
 ...
 case 'mcp:get-server-config': { const stored = await chrome.storage.local.get(['mcpServerUrl', 'mcpConnectionType'
 ...
 case 'mcp:update-server-config': {
        const { config
        } = payload; if (!config || typeof config.uri !== 'string') { throw new Error('Invalid server config: uri is required');
        }
        // Auto-detect connection type from URI if not specified
        let newType = config.connectionType as ConnectionType;
        logger.debug(`Received connection type: ${config.connectionType
        }, parsed as: ${newType
        }`);
        if (!newType) {
          try {
            const url = new URL(config.uri); newType = (url.protocol === 'ws:' || url.protocol === 'wss:') ? 'websocket' : 'sse';
          } catch {
            newType = connectionType; // fallback to current type
          }
        }
        logger.debug(`Updating server config to: ${config.uri
        } (${newType
        })`);
        ...
        const reconnectPromise = (async () => {
          try {
            ...
            if (isConnected) {
              try {
                const primitives = await getPrimitivesWithBackwardsCompatibility(config.uri,
                true, newType);
                const tools = normalizeTools(primitives);
                broadcastToolsUpdateToContentScripts(tools);
                logger.debug(`Broadcasted ${tools.length
                } normalized tools after config update`);
              } catch (toolError) {
 ...
 case 'mcp:heartbeat': {
    ...
    logger.debug(`MCP message ${messageType
    ...
    logger.error(`MCP message handling error (${processingTime
    }ms):`, error);
    ...
    sendResponse({ 
      ...
      success: false,
      timestamp: Date.now(),
      processingTime, origin: 'background',
      id: message.id
    });
  }
}
 ...
 */
function broadcastConnectionStatusToContentScripts(isConnected: boolean, error?: string) { const status = error ? 'error' : (isConnected ? 'connected' : 'disconnected');
  
  logger.debug(`Broadcasting connection status: ${status
  ...
  chrome.tabs.query({}, (tabs) => {
    tabs.forEach(tab => {
      if (tab.id) {
        chrome.tabs.sendMessage(tab.id, broadcastMessage).catch(() => { // Ignore errors for tabs that can't receive messages
        });
      }
    });
  });
...
function broadcastToolsUpdateToContentScripts(tools: any[]) {
  logger.debug(`Broadcasting tools update to content scripts: ${tools.length
  } tools`);
...
async function handleRemoteConfigMessage(
  ...
  try {
    ...
    switch (message.type) { case 'remote-config:fetch': {
        const { force = false
        } = message.payload || {};
 ...
 case 'remote-config:get-feature-flag': {
        ...
        if (!flagName) { throw new Error('Feature flag name is required');
        }
 ...
 case 'remote-config:get-config': {
        ...
        } else { logger.debug('[Background] Getting all remote config');
          result = await remoteConfigManager.getAllConfig();
        }
        //development // logger.debug('[Background] Remote config retrieved:', result);
        break;
      }
 case 'remote-config:get-status': { logger.debug('[Background] Getting remote config status');
        result = {
 ...
 case 'remote-config:clear-cache': { logger.debug('[Background] Clearing remote config cache and refreshing');
    ...
    logger.debug(`Remote Config message processed successfully: ${message.type
    } (${response.processingTime
    }ms)`);
    sendResponse(response);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error(`Error processing Remote Config message ${message.type
    }:`, error);
...
async function initializeRemoteConfig(): Promise<void> {
  try {
    ...
    await remoteConfigManager.initialize(); logger.debug('[Background] Remote Config Manager initialized successfully');
    
    // Make RemoteConfigManager globally accessible for testing if (typeof globalThis !== 'undefined') {
      (globalThis as any).remoteConfigManager = remoteConfigManager; logger.debug('[Background] RemoteConfigManager is now accessible globally as window.remoteConfigManager');
    }
  } catch (error) {
       logger.error('[Background] Failed to initialize Remote Config Manager:', error); // Don't throw - let the extension continue without remote config
  }
}