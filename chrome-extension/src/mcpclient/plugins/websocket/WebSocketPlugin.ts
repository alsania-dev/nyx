...
} from '@modelcontextprotocol/sdk/types.js'; import type { ITransportPlugin, PluginMetadata, PluginConfig
} from '../../types/plugin.js';
import type { WebSocketPluginConfig
...
export class WebSocketPlugin implements ITransportPlugin {
  readonly metadata: PluginMetadata = { name: 'WebSocket Transport Plugin', version: '1.0.0', transportType: 'websocket', description: 'WebSocket transport for MCP protocol with real-time bidirectional communication', author: 'Nyx',
  };
  ...
  async initialize(config: PluginConfig): Promise<void> {
    this.config = { protocols: ['mcp-v1'
      ],
      pingInterval: 30000,
      pongTimeout: 5000,
      maxReconnectAttempts: 3, binaryType: 'arraybuffer',
      ...config,
  ...
  async connect(uri: string): Promise<Transport> {
    ...
    try {
      ...
      this.transport = transport as unknown as WebSocketTransport; logger.debug('[WebSocketPlugin] Transport created successfully');
      return transport;
    } catch (error) {
  ...
  private async createConnection(uri: string): Promise<Transport> {
    try {
      ...
      // Ensure WebSocket protocol if (url.protocol !== 'ws:' && url.protocol !== 'wss:') {
      ...
      // Set up event listeners for monitoring transport.on('close', (event: any) => {
        logger.debug(`Transport closed: ${event.code
        } ${event.reason
        }`);
        this.isConnectedFlag = false;
        
        // Notify the main client about disconnection this.handleDisconnection('WebSocket closed', event.code, event.reason);
      });
 transport.on('error', (error: any) => { logger.error('[WebSocketPlugin] Transport error:', error); // Don't immediately mark as disconnected for ping/pong errors
        // Let MCP protocol handle connection management
        if (!error.message.includes('Pong timeout')) {
          this.isConnectedFlag = false; this.handleDisconnection('WebSocket error', undefined, error.message);
        }
      });

      // Return the transport without connecting - the main client will handle connection logger.debug('[WebSocketPlugin] WebSocket transport created successfully');
      return transport as unknown as Transport;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);

      // Enhanced error messages for WebSocket-specific issues
      let enhancedError = errorMessage; if (errorMessage.includes('timeout')) { enhancedError = 'WebSocket connection timeout. The server may be slow or unreachable.'; } else if (errorMessage.includes('Failed to construct')) { enhancedError = 'Invalid WebSocket URL format. Check the URI syntax.'; } else if (errorMessage.includes('connection failed')) { enhancedError = 'WebSocket connection failed. Check if the server is running and accessible.'; } else if (errorMessage.includes('protocol')) { enhancedError = 'WebSocket protocol error. The server may not support the requested protocols.';
      }
  ...
  async disconnect(): Promise<void> { logger.debug('[WebSocketPlugin] Disconnecting...');
 ...
 logger.debug('[WebSocketPlugin] Disconnected');
  ...
  isConnected(): boolean { // The plugin creates transports but doesn't manage connection state
  ...
  async callTool(client: Client, toolName: string, args: any): Promise<any> {
    if (!this.isConnected()) { throw new Error('WebSocket Plugin: Not connected');
    }
 ...
 logger.debug('[WebSocketPlugin] Getting primitives...');
    ...
    try {
      ...
      // Check if this is a connection-related error
      if (!this.isConnected()) {
        this.isConnectedFlag = false; throw new Error('WebSocket connection lost while getting primitives');
      }
  ...
  private handleDisconnection(reason: string, code?: number, details?: string): void {
    logger.debug(`Handling disconnection: ${reason
    } (code: ${code
    }, details: ${details
    })`);
