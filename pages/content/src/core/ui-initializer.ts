...
// Simple logger for UI contexts const logger = createLogger('UILogger');
...
export async function initializeUIApplication(
  appComponent: React.ComponentType, rootElementId: string = 'root', context: string = 'UI'
): Promise<void> {

   logger.debug('UI application initialization started...');
  
  try {
    // Step 1: Initialize core application logger.debug('Initializing core application...');
    await applicationInit(); logger.debug('Core application initialized successfully.');
    ...
    // Step 3: Create React root and render logger.debug('Creating React root and rendering application...');
        ...
        ">
          ...
          <details style="margin-top: 10px;">
            ...
            <pre style="
              ...
              padding: 10px; 
              border-radius: 4px;
              overflow: auto; ">${error instanceof Error ? error.stack : 'No stack trace available'}</pre>
          </details>
        </div>
      `;
    }
 ...
 */
export async function initializePopupApp(AppComponent: React.ComponentType): Promise<void> { return initializeUIApplication(AppComponent, 'root', 'PopupApp');
}

/**
 * Example options page initialization
 */
export async function initializeOptionsApp(AppComponent: React.ComponentType): Promise<void> { return initializeUIApplication(AppComponent, 'root', 'OptionsApp');
}
...
export function setupUICleanup(): void {
  ...
  // Note: This might not always run reliably for popups due to their lifecycle window.addEventListener('beforeunload', () => {
    applicationCleanup().catch(err => { logger.error('Error during UI cleanup:', err);
    });
  });

  // For Chrome extension contexts, also listen for the page hiding window.addEventListener('pagehide', () => {
    applicationCleanup().catch(err => { logger.error('Error during page hide cleanup:', err);
    });
  });
}
