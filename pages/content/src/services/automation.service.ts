...
// Store references for accessing state outside React components const logger = createLogger('AutomationService');
...
async function initializeStoreAccess() {
  try {
    ...
    storeRefs.getCurrentAdapterState = async () => {
      ...
      return {
        plugin, // Bind methods to maintain proper 'this' context
        ...
        isReady: !!plugin &&  activeAdapterRegistration.status === 'active' && 
...
export class AutomationService {
  ...
  public async initialize(): Promise<void> {
    if (this.isInitialized) { logger.debug('[AutomationService] Already initialized, skipping');
      return;
    }
 logger.debug('[AutomationService] Initializing automation service');
    ...
    this.isInitialized = true; logger.debug('[AutomationService] Automation service initialized successfully');
  }
  /**
 ...
 logger.debug('[AutomationService] Cleaning up automation service');
    ...
    this.isInitialized = false; logger.debug('[AutomationService] Automation service cleaned up');
  }
  /**
  ...
  private setupToolExecutionListener(): void {
    ...
    // Add event listener to document document.addEventListener('mcp:tool-execution-complete', this.eventListener); logger.debug('[AutomationService] Tool execution event listener registered');
  }
  /**
   * Set up listener for MCP state changes
   */
  private setupMCPStateListener(): void {
    // Listen for MCP connection state changes via the event bus eventBus.on('connection:status-changed', ({ status
  ...
  private async handleToolExecutionComplete(event: CustomEvent<ToolExecutionCompleteDetail>): Promise<void> {
    if (!event.detail) { logger.warn('[AutomationService] Tool execution complete event received without detail');
      return;
    }

    const detail = event.detail; logger.debug('[AutomationService] Tool execution complete event received:', detail);

    try {
      // Get current automation state from user preferences
      const automationState = await this.getAutomationState();
      
      if (!automationState) { logger.debug('[AutomationService] Could not get automation state, skipping automation');
        return;
      }
 ...
 logger.debug('[AutomationService] Current automation state:', automationState);
      ...
      } else { logger.debug('[AutomationService] Auto Insert disabled, skipping insert and submit actions');
      }
    } catch (error) {
  ...
  private async getAutomationState(): Promise<AutomationState | null> {
    try {
      ...
      if (!storeRefs.getUserPreferences) { logger.error('[AutomationService] Store access not initialized');
        return null;
      }
  ...
  private async handleAutoExecute(detail: ToolExecutionCompleteDetail): Promise<void> {
    ...
    if (delay > 0) {
      logger.debug(`Auto Execute: Waiting ${delay
 ...
 logger.debug('[AutomationService] Auto Execute: Tool execution completed',
    ...
    // Emit event for potential future integrations // eventBus.emit('automation:execute-completed', {
    //   detail,
    //   timestamp: Date.now()
    // });
  }
  ...
  private async handleAutoInsert(detail: ToolExecutionCompleteDetail): Promise<boolean> {
    ...
    if (delay > 0) {
      logger.debug(`Auto Insert: Waiting ${delay
      } seconds before insertion`);
      await new Promise(resolve => setTimeout(resolve, delay * 1000));
    }
 ...
 // Additional safety check: Don't auto-insert if skipAutoInsertCheck is true
    ...
    try {
      ...
      if (!storeRefs.getCurrentAdapterState) { logger.error('[AutomationService] Adapter store access not initialized');
        return false;
      }

      const { plugin: activePlugin, insertText, attachFile, isReady
      } = await storeRefs.getCurrentAdapterState();

      if (!isReady || !activePlugin) { logger.warn('[AutomationService] No active adapter available for auto insert');
        return false;
      }
 logger.debug('[AutomationService] Using adapter for auto insert:', activePlugin.name);

      // Handle file attachment
      if (detail.isFileAttachment && detail.file && attachFile) { logger.debug('[AutomationService] Auto inserting file:', detail.file.name);
        
        try {
          const success = await attachFile(detail.file);
          
          if (success) { logger.debug('[AutomationService] File attached successfully via auto insert');
            
            // Optionally insert confirmation text if provided
            if (detail.confirmationText && insertText) { logger.debug('[AutomationService] Inserting file confirmation text');
       ...
       logger.error('[AutomationService] Error inserting confirmation text:', error);
            ...
            return true;
          } else { logger.warn('[AutomationService] File attachment failed');
            return false;
          }
        } catch (attachError) {
       logger.error('[AutomationService] Error calling attachFile method:', attachError); logger.error('[AutomationService] attachFile context info:',
          {
      ...
      else if (detail.result && insertText) { logger.debug('[AutomationService] Auto inserting text result');
        
        try {
          const success = await insertText(detail.result);
          
          if (success) { logger.debug('[AutomationService] Text inserted successfully via auto insert');
            return true;
          } else { logger.warn('[AutomationService] Text insertion failed');
            return false;
          }
        } catch (insertError) {
       logger.error('[AutomationService] Error calling insertText method:', insertError); logger.error('[AutomationService] insertText context info:',
          {
      ...
      else { logger.warn('[AutomationService] No valid insertion method found for auto insert',
        {
          hasResult: !!detail.result,
          isFileAttachment: detail.isFileAttachment,
          hasFile: !!detail.file,
  ...
  private async handleAutoSubmit(detail: ToolExecutionCompleteDetail): Promise<boolean> {
    ...
    if (delay > 0) {
      logger.debug(`Auto Submit: Waiting ${delay
 ...
 logger.debug('[AutomationService] Handling auto submit',
    ...
    try {
      ...
      const { plugin: activePlugin, submitForm, isReady
      } = await storeRefs.getCurrentAdapterState();

      if (!isReady || !activePlugin || !submitForm) { logger.warn('[AutomationService] No active adapter or submit capability available for auto submit');
        return false;
      }
 logger.debug('[AutomationService] Using adapter for auto submit:', activePlugin.name);
      ...
      try {
        ...
        if (success) { logger.debug('[AutomationService] Form submitted successfully via auto submit');
          return true;
        } else { logger.warn('[AutomationService] Form submission failed');
          return false;
        }
      } catch (submitError) {
       logger.error('[AutomationService] Error calling submitForm method:', submitError); logger.error('[AutomationService] submitForm context info:',
        {
   ...
   */
  public async triggerTestAutomation(detail: ToolExecutionCompleteDetail): Promise<void> { logger.debug('[AutomationService] Triggering test automation'); await this.handleToolExecutionComplete(new CustomEvent('mcp:tool-execution-complete',
    { detail
    }));
  }
  /**
  ...
  private async exposeAutomationStateToWindow(): Promise<void> {
    try {
      const automationState = await this.getAutomationState();
      if (automationState) {
        (window as any).__mcpAutomationState = automationState; logger.debug('[AutomationService] Exposed automation state to window:', automationState);
      }
    } catch (error) {
...
export default automationService; // Development utilities
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  ...
  (window as any).__automationService = {
    ...
    testAutoSubmit: async () => {
      return automationService.triggerTestAutomation({ result: 'Test result for auto submit',
        isFileAttachment: false,
      ...
      return automationService.triggerTestAutomation({
        ...
        file,
        fileName,
        confirmationText: `File ${fileName
