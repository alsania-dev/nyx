...
import { createLogger } from '@extension/shared/lib/logger'; const logger = createLogger('AnalyticsListener');

/**
 * Initialize analytics listeners for content script events
 */
export function initializeAnalyticsListeners(): void { logger.debug('[AnalyticsListener] Initializing event listeners...');

  // Track adapter activation eventBus.on('adapter:activated', async (data) => {
    try {
 ...
 logger.debug('[AnalyticsListener] Tracked adapter activation:', data.pluginName);
  ...
  // Track feature usage from UI events eventBus.on('ui:sidebar-toggle', async (data) => {
    try {
      await analyticsService.trackFeatureUsage({ feature_name: 'sidebar_toggle', interaction_type: 'click',
 ...
 eventBus.on('ui:sidebar-minimize', async (data) => {
    try {
      await analyticsService.trackFeatureUsage({ feature_name: 'sidebar_minimize', interaction_type: 'click',
  ...
  // Track errors with enhanced context eventBus.on('error:unhandled', async (data) => {
    try {
  ...
  // Track session summary on app shutdown eventBus.on('app:shutdown', async () => {
    try {
      await analyticsService.trackSessionSummary(); logger.debug('[AnalyticsListener] Tracked session summary on shutdown');
    } catch (error) {
 ...
 */
function determineErrorCategory( context?: string | Record<string, any>
): 'connection' | 'tool_execution' | 'adapter' | 'ui' | 'unknown' { if (!context) return 'unknown';
 const contextStr = typeof context === 'string' ? context : JSON.stringify(context);
  const lowerContext = contextStr.toLowerCase();
 if (lowerContext.includes('connection') || lowerContext.includes('connect')) { return 'connection';
  } if (lowerContext.includes('tool') || lowerContext.includes('execute')) { return 'tool_execution';
  } if (lowerContext.includes('adapter') || lowerContext.includes('plugin')) { return 'adapter';
  } if (lowerContext.includes('ui') || lowerContext.includes('sidebar') || lowerContext.includes('button')) { return 'ui';
  }
 return 'unknown';
}
...
export function startPeriodicSessionTracking(): void {
  ...
  sessionSummaryInterval = window.setInterval(async () => {
    try {
      await analyticsService.trackSessionSummary(); logger.debug('[AnalyticsListener] Tracked periodic session summary');
    } catch (error) {
...
export function stopPeriodicSessionTracking(): void {
  if (sessionSummaryInterval) {
    clearInterval(sessionSummaryInterval);
    sessionSummaryInterval = null; logger.debug('[AnalyticsListener] Stopped periodic session tracking');
  }
}
