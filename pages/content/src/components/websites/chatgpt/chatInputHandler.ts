...
// Cache for the last found input element to improve reliability const logger = createLogger('ChatInputHandler');
 ...
 */
export const findChatInputElement = (): HTMLElement | null => { // Try to find ChatGPT's contenteditable div
  const chatGptInput = document.querySelector(
    'div[contenteditable="true"][translate="no"][class="ProseMirror"][id="prompt-textarea"]',
  );

  if (chatGptInput) { logMessage('Found ChatGPT input element');
  ...
  // Fallback: Try to find the input element using the placeholder element logMessage('Primary selector failed, trying fallback method'); const placeholderElement = document.querySelector('p[data-placeholder="Ask anything"].placeholder');
    ...
    while (parent) { if (parent.getAttribute('contenteditable') === 'true') { logMessage('Found ChatGPT input element via placeholder parent');
    ...
    if (parentSiblings) {
      for (let i = 0; i < parentSiblings.length; i++) {
        const sibling = parentSiblings[i]; if (sibling.getAttribute('contenteditable') === 'true') { logMessage('Found ChatGPT input element via placeholder sibling');
  ...
  // Second fallback: Try to find the textarea element with placeholder "Ask anything..." logMessage('First fallback failed, trying textarea selector'); const textareaElement = document.querySelector('textarea[placeholder="Ask anything..."]');
  
    if (textareaElement) { logMessage('Found ChatGPT input element via textarea selector');
...
export const insertTextToChatInput = (text: string): boolean => {
  try {
    ...
    if (!chatInput) { logMessage('Could not find ChatGPT input element'); logger.error('Could not find ChatGPT input element');
      return false;
    }
 ...
 logMessage('Appended text using textContent (fallback method)');
    ...
    logMessage(`Error appending text to ChatGPT input: ${errorMessage}`); logger.error('Error appending text to ChatGPT input:', error);
    return false;
  }
};
...
export const attachFileToChatInput = async (file: File): Promise<boolean> => {
  try {
    ...
    if (!chatInput) { logMessage('Could not find ChatGPT input element for file attachment');
      return false;
    }
 ...
 const dropEvent = new DragEvent('drop', {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logMessage(`Error attaching file to ChatGPT input: ${errorMessage}`); logger.error('Error attaching file to ChatGPT input:', error);
    return false;
  }
};
    ...
    try {
      ...
      const findSubmitButton = (): HTMLButtonElement | null => {
        const submitButton = document.querySelector('button[aria-label="Send message"]') || document.querySelector('button[data-testid="send-button"]') || document.querySelector('button[aria-label="Send prompt"]') ||
          // Try to find button with paper airplane icon (common pattern in ChatGPT) document.querySelector('button svg[data-icon="paper-airplane"]')?.closest('button') || document.querySelector('button svg path[d*="M12 3.5"]')?.closest('button') ||
