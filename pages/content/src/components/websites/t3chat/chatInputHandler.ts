...
// Cache for the last found input element to improve reliability const logger = createLogger('T3ChatInputHandler');
 ...
 */
export const findChatInputElement = (): HTMLElement | null => { // Try to find T3 Chat's textarea based on the provided HTML
  const t3ChatInput = document.querySelector('textarea#chat-input, textarea[placeholder="Type your message here..."]');

  if (t3ChatInput) { logMessage('Found T3 Chat input element');
  ...
  const possibleInputSelectors = [ 'textarea.resize-none', 'textarea[aria-label="Message input"]', 'div[role="textbox"]', 'div.chat-input', 'textarea[data-testid="chat-input"]', 'div[contenteditable="true"]', 'textarea.message-input',
  ];
...
export const insertTextToChatInput = (text: string): boolean => {
  try {
    ...
    if (!chatInput) { logMessage('Could not find T3 Chat input element'); logger.error('Could not find T3 Chat input element');
      return false;
    }
 ...
 logMessage('Appended text to element via textContent property');
    ...
    logMessage(`Error inserting text into T3 Chat input: ${error}`); logger.error('Error inserting text into T3 Chat input:', error);
    return false;
  }
};
 ...
 * @returns True if successful, false otherwise
 */
export const insertToolResultToChatInput = (result: string): boolean => { logMessage('Inserting tool result to T3 Chat input');
...
export const attachFileToChatInput = (file: File): boolean => {
  ...
  try {
    // Find file input element const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement;
      ...
      if (!fileInput) { logMessage('Could not find file input element in T3 Chat');
      return false;
    }
    ...
    logMessage(`Error attaching file to T3 Chat: ${error}`); logger.error('Error attaching file to T3 Chat:', error);
    return false;
  }
};
...
export const submitChatInput = async (maxWaitTime = 5000): Promise<boolean> => {
  try {
    ...
    let chatInput: HTMLElement | null = null; if (window.location.hostname === 't3.chat') {
      chatInput = findChatInputElement();

      if (!chatInput) { logMessage('Could not find T3 Chat input element for submission');
        return false;
      }
    }
 ...
 logMessage('Could not find a way to submit the T3 Chat input');
    ...
    logMessage(`Error submitting T3 Chat input: ${error}`); logger.error('Error submitting T3 Chat input:', error);
    return false;
  }
};
