...
// Cache for the last found input element to improve reliability const logger = createLogger('OpenRouterChatInputHandler');
 ...
 */
export const findChatInputElement = (): HTMLElement | null => { // Try to find OpenRouter's contenteditable div
  const chatInput = document.querySelector('textarea[name="Chat Input"][placeholder="Start a message..."].w-full');

  if (chatInput) { logMessage('Found OpenRouter input element');
    lastFoundInputElement = chatInput as HTMLElement;
    return chatInput as HTMLElement;
  }

  // Fallback: Try to find the input element using the placeholder element logMessage('Primary selector failed, trying fallback method'); const placeholderElement = document.querySelector('p[data-placeholder="Ask anything"].placeholder');
  
    if (placeholderElement) { logMessage('Found placeholder element, looking for parent input element');
    ...
    while (parent) { if (parent.getAttribute('contenteditable') === 'true') { logMessage('Found OpenRouter input element via placeholder parent');
    ...
    if (parentSiblings) {
      for (let i = 0; i < parentSiblings.length; i++) {
        const sibling = parentSiblings[i]; if (sibling.getAttribute('contenteditable') === 'true') { logMessage('Found OpenRouter input element via placeholder sibling');
  ...
  // Second fallback: Try to find the textarea element with placeholder "Start a message..." logMessage('First fallback failed, trying textarea selector'); const textareaElement = document.querySelector('textarea[placeholder="Start a message..."]');
  
    if (textareaElement) { logMessage('Found OpenRouter input element via textarea selector');
...
export const insertTextToChatInput = (text: string): boolean => {
  try {
    ...
    if (!chatInput) { logMessage('Could not find OpenRouter input element'); logger.error('Could not find OpenRouter input element');
      return false;
    }
 ...
 logMessage('Appended text to textarea with preserved newlines');
      ...
      // Get current content const currentText = chatInput.textContent || '';
 ...
 // If there's existing content, add newlines before the new text
      if (currentText && currentText.trim() !== '') {
        ...
        if (!chatInput.lastChild || chatInput.lastChild.nodeType !== Node.TEXT_NODE) { chatInput.appendChild(document.createTextNode(''));
        }
 ...
 logMessage('Appended text using textContent (fallback method)');
    ...
    logMessage(`Error appending text to OpenRouter input: ${errorMessage}`); logger.error('Error appending text to OpenRouter input:', error);
    return false;
  }
};
...
export const insertToolResultToChatInput = (result: any): boolean => {
  try {
    ...
    // const wrappedResult = wrapInToolOutput(formattedResult); // Convert result to string if it's not already a string
...
export const attachFileToChatInput = async (file: File): Promise<boolean> => {
  try {
    ...
    if (!chatInput) { logMessage('Could not find OpenRouter input element for file attachment');
      return false;
    }
 ...
 const dropEvent = new DragEvent('drop', {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logMessage(`Error attaching file to OpenRouter input: ${errorMessage}`); logger.error('Error attaching file to OpenRouter input:', error);
    return false;
  }
};
...
export const submitChatInput = (maxWaitTime = 5000): Promise<boolean> => {
  return new Promise(resolve => {
    try {
      ...
      if (!chatInput) { logMessage('Could not find OpenRouter chat input to submit');
        resolve(false);
        return;
      }
      ...
      const findSubmitButton = (): HTMLButtonElement | null => {
        const submitButton = document.querySelector('button[aria-label="Send message"]') || document.querySelector('button[data-testid="send-button"]') || document.querySelector('button[aria-label="Send prompt"]') ||
          // Try to find button with paper airplane icon (common pattern in OpenRouter) document.querySelector('button svg[data-icon="paper-airplane"]')?.closest('button') || document.querySelector('button svg path[d*="M12 3.5"]')?.closest('button') ||
          // Look for any button near the input area that looks like a submit button chatInput.closest('form')?.querySelector('button[type="submit"]') || chatInput.closest('div')?.querySelector('button:last-child');
      ...
      if (submitButton) { logMessage(`Found submit button (${submitButton.getAttribute('aria-label') || 'unknown'})`);

        // Function to check if button is enabled and click it
        const tryClickingButton = () => {
          const button = findSubmitButton();
          if (!button) { logMessage('Submit button no longer found');
            resolve(false);
            return;
          }
          ...
          if (!isDisabled) { logMessage('Submit button is enabled, clicking it');
            button.click();
            resolve(true);
          } else { logMessage('Submit button is disabled, waiting...');
          }
        };
 ...
 // If we've waited too long, try alternative methods
          if (elapsedTime >= maxWaitTime) {
            ...
            if (form) { logMessage('Found form element, submitting it'); const submitEvent = new SubmitEvent('submit', { bubbles: true, cancelable: true });
              form.dispatchEvent(submitEvent);
              resolve(true);
              return;
            }

            // Method 3: Simulate Enter key press logMessage('Simulating Enter key press as fallback');
 ...
 // Initial check - maybe it's already enabled
        ...
        // Method 2: Look for the form and submit it directly const form = chatInput.closest('form');
        if (form) { logMessage('Found form element, submitting it'); const submitEvent = new SubmitEvent('submit', { bubbles: true, cancelable: true });
          form.dispatchEvent(submitEvent);
          resolve(true);
          return;
