...
// Cache for the last found input element to improve reliability const logger = createLogger('KagiChatInputHandler');
 ...
 */
export const findChatInputElement = (): HTMLElement | null => { // Try to find Kagi's textarea or contenteditable div
  // Note: These selectors may need to be updated based on Kagi's actual DOM structure
  const kagiInput = document.querySelector( 'textarea[aria-label="Ask Kagi anything"], textarea[placeholder="Ask anything"], textarea[placeholder], textarea[spellcheck="false"], textarea[data-gramm="false"], div.css-146c3p1 textarea, textarea.r-30o5oe, div[contenteditable="true"]',
  );

  if (kagiInput) { logMessage('Found Kagi input element');
  ...
  const possibleInputSelectors = [ 'textarea.chat-input', 'div[role="textbox"]', 'div.chat-input', 'textarea[data-testid="chat-input"]', 'div[contenteditable="true"]', 'textarea.message-input', 'textarea[aria-label="Ask Kagi anything"]',
  ];
...
export const insertTextToChatInput = (text: string): boolean => {
  try {
    ...
    if (!chatInput) { logMessage('Could not find Kagi input element'); logger.error('Could not find Kagi input element');
      return false;
    }
 ...
 logMessage('Appended text to element via textContent property');
    ...
    logMessage(`Error inserting text into Kagi input: ${error}`); logger.error('Error inserting text into Kagi input:', error);
    return false;
  }
};
 ...
 * @returns True if successful, false otherwise
 */
export const insertToolResultToChatInput = (result: string): boolean => { logMessage('Inserting tool result to Kagi chat input');
...
export const attachFileToChatInput = (file: File): boolean => {
  ...
  try {
    // Find file input element const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement;
      ...
      if (!fileInput) { logMessage('Could not find file input element in Kagi');
      return false;
    }
    ...
    logMessage(`Error attaching file to Kagi chat: ${error}`); logger.error('Error attaching file to Kagi chat:', error);
    return false;
  }
};
...
export const submitChatInput = async (maxWaitTime = 5000): Promise<boolean> => {
  try {
    ...
    let chatInput: HTMLElement | null = null; if (window.location.hostname === 'kagi.com') {
      chatInput = findChatInputElement();

      if (!chatInput) { logMessage('Could not find Kagi input element for submission');
        return false;
      }
    }
 ...
 // If the keydown event didn't trigger submission (it was prevented),
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        if (!chatInput) { logMessage('No chat input found for form submission');
          resolve(false);
          return;
        }
 const form = chatInput.closest('form');
        if (form) { logMessage('Found form, submitting it'); form.dispatchEvent(new SubmitEvent('submit', { bubbles: true, cancelable: true }));
          resolve(true);
          return;
        }
 logMessage('Could not find a way to submit the Kagi chat input');
    ...
    logMessage(`Error submitting Kagi chat input: ${error}`); logger.error('Error submitting Kagi chat input:', error);
    return false;
  }
};
