...
// Cache for the last found input element to improve reliability const logger = createLogger('GrokChatInputHandler');
 ...
 */
export const findChatInputElement = (): HTMLElement | null => { // Try to find Grok's textarea or contenteditable div
  // Note: These selectors may need to be updated based on Grok's actual DOM structure
  const grokInput = document.querySelector( 'textarea[aria-label="Ask Grok anything"], textarea[placeholder="Ask anything"], textarea[placeholder], textarea[spellcheck="false"], textarea[data-gramm="false"], div.css-146c3p1 textarea, textarea.r-30o5oe, div[contenteditable="true"]',
  );

  if (grokInput) { logMessage('Found Grok input element');
  ...
  const possibleInputSelectors = [ 'textarea.chat-input', 'div[role="textbox"]', 'div.chat-input', 'textarea[data-testid="chat-input"]', 'div[contenteditable="true"]', 'textarea.message-input', 'textarea[aria-label="Ask Grok anything"]',
  ];
...
export const insertTextToChatInput = (text: string): boolean => {
  try {
    ...
    if (!chatInput) { logMessage('Could not find Grok input element'); logger.error('Could not find Grok input element');
      return false;
    }
 ...
 logMessage('Appended text to element via textContent property');
    ...
    logMessage(`Error inserting text into Grok input: ${error}`); logger.error('Error inserting text into Grok input:', error);
    return false;
  }
};
 ...
 * @returns True if successful, false otherwise
 */
export const insertToolResultToChatInput = (result: string): boolean => { logMessage('Inserting tool result to Grok chat input');
...
export const attachFileToChatInput = (file: File): boolean => {
  ...
  try {
    // Find file input element const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement;
      ...
      if (!fileInput) { logMessage('Could not find file input element in Grok');
      return false;
    }
    ...
    logMessage(`Error attaching file to Grok chat: ${error}`); logger.error('Error attaching file to Grok chat:', error);
    return false;
  }
};
...
export const submitChatInput = async (maxWaitTime = 5000): Promise<boolean> => {
  try {
    ...
    let chatInput: HTMLElement | null = null; if (window.location.hostname === 'grok.com') {
      chatInput = findChatInputElement();

      if (!chatInput) { logMessage('Could not find Grok input element for submission');
        return false;
      }
    }
 ...
 logMessage('Could not find a way to submit the Grok chat input');
    ...
    logMessage(`Error submitting Grok chat input: ${error}`); logger.error('Error submitting Grok chat input:', error);
    return false;
  }
};
