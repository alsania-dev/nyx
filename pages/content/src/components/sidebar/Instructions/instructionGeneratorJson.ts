...
import { jsonSchemaToCsn
} from './schema_converter';
import { chatgptInstructions
} from './website_specific_instruction/chatgpt';
import { geminiInstructions
} from './website_specific_instruction/gemini';
import { createLogger
...
- Each function call is a JSON Lines object with "name",
  "call_id", and "parameters" properties
- Parameters are provided as a JSON Lines object with parameter names as keys
- Required parameters must always be included
- Optional parameters should only be included when needed The instructions regarding function calls specify that:
...
- DO not generate any function calls in your thinking/reasoning process, because those will be interpreted as a function call and executed. Just formulate the correct parameters for the function call. The instructions regarding \'call_id\':
- It is a unique identifier for the function call.
- It is a number that is incremented by 1 for each new function call, starting from 1.
...
{
    "type": "description",
    "text": "Short 1 line of what this function does"
  }
{
    "type": "parameter",
    "key": "parameter_1",
...
\`\`\`
</example_function_call> When a user makes a request:
1. ALWAYS analyze what function calls would be appropriate for the task
2. ALWAYS format your function call usage EXACTLY as specified in the schema
3. NEVER skip required parameters in function calls
...
`;
  ...
  const currentHost = window.location.hostname; if (currentHost.includes('gemini')) {
    instructions += geminiInstructions;
  }
  //# ChatGPT-Specific Instructions if (currentHost.includes('chatgpt')) {
...
ap f | Additional properties false | o {p {name:s
    } ap f
  }
type(key=value, ...) | Constrained type | name:s(minLength=1)
a[type
  ] | Array with item type | tags:a[s
  ]
o {p {prop:type
    }
  } | Nested object | user:o {p {id:i; name:s
    }
  }
...
`;
  ...
  // instructions += '';
  // // Add compressed schemas section // instructions += '## Tools and their schema\n\n';
  // // Add each tool with its compressed schema
  // tools.forEach(tool => {
  //   try {
  //     const schema = JSON.parse(tool.schema); //     let compressedSchema = '';
  //     try {
  //       compressedSchema = jsonSchemaToCsn(schema);
  //     } catch (error) {
  //       logger.error(`Error compressing schema for ${tool.name}:`, error); //       compressedSchema = 'Schema conversion failed';
  //     }
  ...
  //   }
  // }); // instructions += '\n';
  // Add available tools section instructions += '## AVAILABLE TOOLS FOR NYX\n\n';
  ...
  tools.forEach(tool => {
    ...
    try {
      ...
      if (schema.properties && Object.keys(schema.properties).length > 0) { instructions += '**Parameters**:\n';
        ...
        Object.entries(schema.properties).forEach(([paramName, paramDetails
          ...
          // Handle nested objects if (paramDetails.type === 'object' && paramDetails.properties) { instructions += '  - Properties:\n';
          ...
          if ( paramDetails.type === 'array' &&
            paramDetails.items && paramDetails.items.type === 'object' &&
 ...
 instructions += '\n';
      ...
      // Add example usage // instructions += '**Example Usage**:\n\n'; // instructions += '```\n<use_mcp_tool>\n{\n';
      // instructions += `  "tool": "${tool.name}",\n`; // instructions += '  "args": {\n';
      // Add example parameters based on schema
      // if (schema.properties) {
      //   const exampleParams = Object.entries(schema.properties).map(([paramName, paramDetails]: [string, any]) => { //     let exampleValue = '';
      //     // Generate example value based on type
      //     switch (paramDetails.type) { //       case 'string':
      //         exampleValue = paramDetails.example || `"example_${paramName}"`;
      //         break; //       case 'number': //         exampleValue = paramDetails.example || '42';
      //         break; //       case 'boolean': //         exampleValue = paramDetails.example || 'true';
      //         break; //       case 'array': //         exampleValue = paramDetails.example || '[]';
      //         break; //       case 'object': //         exampleValue = paramDetails.example || '{}';
      //         break;
      ...
      // If schema parsing fails, provide a simpler example instructions += 'Schema information not available. No Tools Available'; // instructions += '```\n<use_mcp_tool>\n{\n';
      // instructions += `  "tool": "${tool.name}",\n`; // instructions += '  "args": {}\n'; // instructions += '}\n</use_mcp_tool>\n```\n\n';
    }
  });
