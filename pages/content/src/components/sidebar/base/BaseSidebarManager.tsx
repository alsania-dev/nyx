...
import {
  ...
  applyDarkMode,
  applyLightMode, injectTailwindToShadowDom,
 ...
 */
export type SiteType = | 'perplexity' | 'chatgpt' | 'grok' | 'zai' | 'gemini' | 'aistudio' | 'openrouter' | 'deepseek' | 'kagi' | 't3chat';
...
export abstract class BaseSidebarManager {
  ...
  /**
   * Apply theme class to the Shadow DOM host element * @param theme The theme to apply: 'light', 'dark', or 'system'
   * @returns Whether the theme was successfully applied
   */ public applyThemeClass(theme: 'light' | 'dark' | 'system'): boolean {
    if (!this.shadowHost || !this.shadowRoot) { logMessage('[BaseSidebarManager] Cannot apply theme: Shadow host or root not found.');
      return false;
    }
 const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;

    // Remove existing theme classes first this.shadowHost.classList.remove('light', 'dark');
 if (theme === 'dark' || (theme === 'system' && prefersDark)) { this.shadowHost.classList.add('dark');
    ...
    } else { this.shadowHost.classList.add('light');
  ...
  public setPushContentMode(enabled: boolean, sidebarWidth?: number, isCollapsed?: boolean): void {
    ...
    if (enabled) {
      ...
      const width = isCollapsed ? 56 : sidebarWidth || 320; document.documentElement.style.setProperty('--sidebar-width-mcp', `${width}px`);

      // Apply specific inline styles directly to the HTML element with !important document.documentElement.style.setProperty('position', 'relative', 'important'); document.documentElement.style.setProperty('margin-right', `${width}px`, 'important'); document.documentElement.style.setProperty('width', `calc(100% - ${width}px)`, 'important'); document.documentElement.style.setProperty('min-height', '100vh', 'important'); document.documentElement.style.setProperty('box-sizing', 'border-box', 'important');

      // Also apply styles to body for better compatibility with different website layouts document.body.style.setProperty('margin-right', '0', 'important'); document.body.style.setProperty('padding-right', '0', 'important'); document.body.style.setProperty('box-sizing', 'border-box', 'important'); document.body.style.setProperty('max-width', 'none', 'important');

      // Add classes to HTML root for CSS-based layout adjustments document.documentElement.classList.add('push-mode-enabled');
      ...
      // When push mode is enabled, ensure the sidebar is visible if (!this._isVisible || (this.shadowHost && this.shadowHost.style.display !== 'block')) { logMessage('[BaseSidebarManager] Push mode enabled but sidebar not visible, showing sidebar');
        // Use a non-async version of show to ensure immediate visibility
        this.forceVisibility();
      }
    } else {
      // Remove all inline styles when push mode is disabled document.documentElement.style.removeProperty('position'); document.documentElement.style.removeProperty('margin-right'); document.documentElement.style.removeProperty('width'); document.documentElement.style.removeProperty('min-height'); document.documentElement.style.removeProperty('box-sizing'); document.documentElement.style.removeProperty('transform');

      // Remove body styles document.body.style.removeProperty('margin-right'); document.body.style.removeProperty('padding-right'); document.body.style.removeProperty('box-sizing'); document.body.style.removeProperty('max-width');

      // Remove push mode classes when disabled document.documentElement.classList.remove('push-mode-enabled', 'sidebar-collapsed', 'push-mode-transform');
    }
 ...
 logMessage(`BaseSidebarManager: Push mode ${enabled ? 'enabled' : 'disabled'}`);
  ...
  private forceVisibility(): void {
    if (!this._isInitialized) { logMessage('[BaseSidebarManager] Cannot force visibility: not initialized');
      return;
    }

    if (!this.shadowHost) { logMessage('[BaseSidebarManager] Cannot force visibility: no shadow host');
      return;
    }

    this._isVisible = true; this.shadowHost.style.display = 'block'; this.shadowHost.style.opacity = '1'; this.shadowHost.classList.add('initialized');

    // Remove any transition classes to ensure immediate visibility this.shadowHost.classList.remove('showing');
 ...
 logMessage('[BaseSidebarManager] Forced sidebar visibility');
  ...
  public updatePushModeStyles(width: number): void {
    if (this._isPushContentMode) { document.documentElement.style.setProperty('--sidebar-width-mcp', `${width}px`); document.documentElement.style.setProperty('margin-right', `${width}px`, 'important'); document.documentElement.style.setProperty('width', `calc(100% - ${width}px)`, 'important');
   ...
   */
  public removePushModeStyles(): void { document.documentElement.style.removeProperty('position'); document.documentElement.style.removeProperty('margin-right'); document.documentElement.style.removeProperty('width'); document.documentElement.style.removeProperty('min-height'); document.documentElement.classList.remove('push-mode-enabled', 'sidebar-collapsed');
  }

  /**
   * Ensures that push mode styles are added to the document
   */
  private ensurePushModeStyles(): void { if (!document.getElementById('mcp-sidebar-push-styles')) { const styleEl = document.createElement('style'); styleEl.id = 'mcp-sidebar-push-styles';
      styleEl.textContent = `
  ...
  public async initialize(): Promise<void> {
    ...
    if (this._initializationPromise) { logMessage('Sidebar initialization already in progress, waiting...');
      return this._initializationPromise;
    }

    // Start new initialization process logMessage('Starting sidebar manager initialization...');
    this._initializationPromise = new Promise<void>(async (resolve, reject) => {
      try {
        // For maximum compatibility, only require document to exist // Don't wait for body if it's not ready - we'll append later
        if (!document.body) { logMessage('[BaseSidebarManager] Document body not ready, waiting briefly...');
          await new Promise<void>(resolve => {
            const checkBody = () => {
              if (document.body) {
                resolve(); } else if (document.readyState === 'complete' || document.readyState === 'interactive') { // If document is ready but body still doesn't exist, something is wrong
                // Create a minimal body to proceed
                if (!document.body) {
                  document.body = document.createElement('body'); logMessage('[BaseSidebarManager] Created minimal document body');
                }
                resolve();
              } else {
        ...
        // Create and setup the shadow host this.shadowHost = document.createElement('div'); this.shadowHost.id = 'mcp-sidebar-shadow-host'; this.shadowHost.style.position = 'fixed'; this.shadowHost.style.top = '0'; this.shadowHost.style.right = '0'; this.shadowHost.style.zIndex = '9999'; this.shadowHost.style.height = '100vh'; this.shadowHost.style.pointerEvents = 'none'; // Allow clicks 'through' the host this.shadowHost.style.display = 'none'; // Initialize as hidden
        // Add specific attributes for Shadow DOM targeting this.shadowHost.setAttribute('data-shadow-host', 'true');
 logMessage('[BaseSidebarManager] Shadow host created and set to display: none.');

        document.body.appendChild(this.shadowHost);

        // Attach shadow root this.shadowRoot = this.shadowHost.attachShadow({ mode: 'open' });

        // Create container for React this.container = document.createElement('div'); this.container.id = 'sidebar-container'; this.container.style.pointerEvents = 'auto'; this.container.style.height = '100%'; this.container.style.width = '100%';
        this.shadowRoot.appendChild(this.container);

        // Inject CSS into Shadow DOM - make this completely non-blocking
        try {
          await injectTailwindToShadowDom(this.shadowRoot); logMessage('[BaseSidebarManager] CSS injection successful');
        } catch (cssError) {
          logMessage(
        ...
        // Create React root
        this.root = createRoot(this.container); logMessage('Sidebar manager initialized with Shadow DOM.');

        // Apply default theme based on system preference - handle failures gracefully
        try { this.applyThemeClass('system');
        } catch (themeError) {
          logMessage(
  ...
  public async show(): Promise<void> {
    ...
    this._isVisible = true; logMessage('[BaseSidebarManager] Show requested, setting _isVisible = true');
    ...
    try {
      ...
      this._isVisible = false; // Reset visibility if init failed return; // Don't proceed if initialization failed
    }
    ...
    if (this.shadowHost) {
      // Start with immediate visibility but with opacity 0 for smooth transition this.shadowHost.style.display = 'block'; this.shadowHost.style.opacity = '0'; this.shadowHost.style.transform = 'translateX(30px) scale(0.95)';

      // Add showing class for CSS animations this.shadowHost.classList.add('showing');
      ...
      requestAnimationFrame(() => {
        if (this.shadowHost) {
          this.shadowHost.style.transition = 'opacity 0.4s cubic-bezier(0.25, 0.8, 0.25, 1), transform 0.4s cubic-bezier(0.25, 0.8, 0.25, 1)'; this.shadowHost.style.opacity = '1'; this.shadowHost.style.transform = 'translateX(0) scale(1)';

          // Add initialized class after animation starts
          setTimeout(() => {
            if (this.shadowHost) { this.shadowHost.classList.add('initialized'); this.shadowHost.classList.remove('showing');
            }
          }, 100);
        }
      });
      ...
      setTimeout(() => {
        this.render(); logMessage('Sidebar shown with enhanced animations');
      }, 50);
    }
  }
   ...
   * Hide the sidebar
   */
  public hide(): void { logMessage('[BaseSidebarManager] Hide method invoked.');

    if (this.shadowHost) {
      // Add hiding class for smooth exit animation this.shadowHost.classList.add('hiding'); this.shadowHost.classList.remove('initialized', 'showing');

      // Start the hiding animation
      this.shadowHost.style.transition = 'opacity 0.3s cubic-bezier(0.55, 0.085, 0.68, 0.53), transform 0.3s cubic-bezier(0.55, 0.085, 0.68, 0.53)'; this.shadowHost.style.opacity = '0'; this.shadowHost.style.transform = 'translateX(20px) scale(0.98)';

      // After animation completes, hide completely
      setTimeout(() => {
        if (this.shadowHost) { this.shadowHost.style.display = 'none'; this.shadowHost.classList.remove('hiding');
          // Reset transform for next show this.shadowHost.style.transform = ''; logMessage('[BaseSidebarManager] Sidebar hidden with smooth animation');
        }
      }, 300);
    }
    ...
    if (previouslyVisible) { logMessage('[BaseSidebarManager] Sidebar hidden (was previously visible).');
    } else { logMessage('[BaseSidebarManager] Sidebar state set to hidden (was already hidden or not shown yet).');
    }
  }
  ...
  protected render(): void {
    if (!this.root || !this.container) { logMessage('Cannot render: Root or container not available.');
      return;
    }

    try {
      const sidebarContent = this.createSidebarContent();
      if (!sidebarContent) { logMessage('createSidebarContent returned null or undefined.');
        this.root.render(null);
      } else { logMessage('Rendering sidebar content into container.');
   ...
   */
  public destroy(): void { window.removeEventListener('mcpToolsUpdated', this.handleToolsUpdated);
 ...
 logMessage('Sidebar manager destroyed');
  ...
  protected handleToolsUpdated = (): void => {
    if (this._isVisible) {
      ...
      // let the React components handle updates through their own state management logMessage('[BaseSidebarManager] Tools updated - letting React components handle updates');
