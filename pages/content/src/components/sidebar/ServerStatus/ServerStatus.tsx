...
const ServerStatus: React.FC<ServerStatusProps> = ({ status: initialStatus }) => {
  ...
  const [isReconnecting, setIsReconnecting] = useState(false); const [lastReconnectTime, setLastReconnectTime] = useState<string>(''); const [serverUri, setServerUri] = useState<string>(serverConfig.uri || ''); const [connectionType, setConnectionType] = useState<ConnectionType>(serverConfig.connectionType || 'sse'); const [statusMessage, setStatusMessage] = useState<string>('');
  ...
  const [isEditingConnectionType, setIsEditingConnectionType] = useState<boolean>(false); const [lastErrorMessage, setLastErrorMessage] = useState<string>('');
  ...
  // Use connection status from store, fallback to prop const status = connectionStatus || initialStatus || 'unknown';
  ...
  const forceReconnect = useCallback(async () => {
    try {
      if (!communicationMethods.forceReconnect) { throw new Error('Communication method unavailable');
      }
  ...
  // Get the last connection error from the background communication hook const backgroundConnectionError = communicationMethods.lastConnectionError || '';
  
    const refreshTools = useCallback(
    async (forceRefresh = false) => {
      try {
        if (!communicationMethods.refreshTools) { throw new Error('Communication method unavailable');
        }
  ...
  const getServerConfig = useCallback(
    async () => {
      try {
        if (!communicationMethods.getServerConfig) { throw new Error('Communication method unavailable');
        }
        ...
        setHasBackgroundError(true); throw error; // Don't fallback to default, let caller handle the error
      }
    },
    [communicationMethods],
  );
  ...
  useEffect(() => {
    const checkImmediateStatus = async () => {
      if (communicationMethods.forceConnectionStatusCheck) {
        try { logMessage('[ServerStatus] Forcing immediate connection status check on mount');
  ...
  useEffect(() => { // During reconnection, don't override the status message
    ...
    if (hasBackgroundError) {
      ...
      switch (status) { case 'connected': setStatusMessage('MCP Server is connected and ready');
          // Brief success indication for new connections
          if (!isConnected) {
            setShowSuccessAnimation(true);
            setTimeout(() => setShowSuccessAnimation(false), 1000);
          }
          break; case 'disconnected': setStatusMessage('MCP Server is unavailable. Some features will be limited.');
          break; case 'error': setStatusMessage(connectionError || 'Error connecting to extension services. Try reloading the page.');
          break;
        default: setStatusMessage('Checking MCP Server status...');
      }
    }
  ...
  useEffect(() => {
    ...
    // Listen for connection status changes from the event bus const unsubscribeConnection = eventBus.on('connection:status-changed', (data) => { logMessage(`[ServerStatus] Connection status event: ${data.status}${data.error ? ` (${data.error})` : ''}`);
 // Update local error state if there's an error
      ...
      // Update last reconnect time for successful connections if (data.status === 'connected') {
    ...
    // Listen for context bridge events const unsubscribeBridgeInvalidated = eventBus.on('context:bridge-invalidated', (data) => {
 ...
 const unsubscribeBridgeRestored = eventBus.on('context:bridge-restored', () => { logMessage('[ServerStatus] Context bridge restored');
      setHasBackgroundError(false); // Don't automatically reconnect here - let user decide
    });
  ...
  useEffect(() => {
    const checkBackgroundAvailability = () => {
      const methodsAvailable = !!( typeof communicationMethods.forceReconnect === 'function' && typeof communicationMethods.refreshTools === 'function' && typeof communicationMethods.getServerConfig === 'function' && typeof communicationMethods.updateServerConfig === 'function'
      );

      if (!methodsAvailable && !hasBackgroundError) {
        setHasBackgroundError(true); setStatusMessage('Extension background services unavailable. Try reloading the page.');
  ...
  useEffect(() => { // Only fetch server configuration on initial mount - don't refetch while user is editing
    const fetchInitialServerConfig = async () => {
      try {
        ...
        } else { logMessage('[ServerStatus] No valid server configuration received from background storage'); setServerUri(''); // Set empty string to indicate no config loaded
          setConfigFetched(true);
        }
      } catch (error) {
        logMessage(
    ...
    if (
      communicationMethods && typeof communicationMethods.getServerConfig === 'function' &&
      !configFetched &&
      !isEditingUri &&
      !isEditingConnectionType
    ) {
      fetchInitialServerConfig().catch(() => { logMessage('[ServerStatus] Failed to fetch server configuration'); setServerUri(''); // Set empty string as last resort
      });
    }
  ...
  useEffect(() => { // During reconnection, don't override the status message set by handleSaveServerConfig
    if (isReconnecting) {
      return;
    }
    ...
    if (hasBackgroundError) {
      ...
      switch (status) { case 'connected': setStatusMessage('MCP Server is connected and ready');
          break; case 'disconnected': setStatusMessage('MCP Server is unavailable. Some features will be limited.');
          break; case 'error': setStatusMessage('Error connecting to extension services. Try reloading the page.');
          break;
        default: setStatusMessage('Checking MCP Server status...');
      }
    }
  ...
  const handleReconnect = async () => {
    ...
    const minDisplayDuration = 1200; // Minimum display time for smooth UX
      ...
      try { logMessage('[ServerStatus] Reconnect button clicked');
      setIsReconnecting(true); setStatusMessage('Attempting to reconnect to MCP server...');

      // Check if we can connect to the background script first
      if (hasBackgroundError) { logMessage('[ServerStatus] Attempting to recover background connection');
        // Wait a bit to see if background services become available
        await new Promise(resolve => setTimeout(resolve, 2000));

        // Check again if background services are available
        if (!communicationMethods.forceReconnect) { throw new Error('Background services still unavailable');
        }
 ...
 logMessage('[ServerStatus] Calling forceReconnect method');
      const success = await forceReconnect(); logMessage(`[ServerStatus] Reconnection ${success ? 'succeeded' : 'failed'}`);
      ...
      if (success) { setStatusMessage('Successfully reconnected to MCP server'); logMessage('[ServerStatus] Reconnection successful, fetching fresh tool list');
        try {
      ...
      } else { setStatusMessage('Failed to reconnect to MCP server. Some features will be limited.');
      }
    } catch (error) {
      ...
      // Display the enhanced error message in the status if (errorMessage.includes('404') || errorMessage.includes('not found')) {
        setStatusMessage( 'Server URL not found (404). Please verify your MCP server URL and ensure the server is running.',
        ); } else if (errorMessage.includes('403')) { setStatusMessage('Access forbidden (403). Please check server permissions and authentication settings.'); } else if (errorMessage.includes('500') || errorMessage.includes('502') || errorMessage.includes('503')) { setStatusMessage('Server error detected. The MCP server may be experiencing issues.'); } else if (errorMessage.includes('Connection refused') || errorMessage.includes('ECONNREFUSED')) { setStatusMessage('Connection refused. Please verify the MCP server is running at the configured URL.'); } else if (errorMessage.includes('timeout') || errorMessage.includes('ETIMEDOUT')) { setStatusMessage('Connection timeout. The server may be slow to respond or unreachable.'); } else if (errorMessage.includes('ENOTFOUND')) { setStatusMessage('Server not found. Please check the server URL and your network connection.');
      } else {
  ...
  const handleDetails = () => {
    ...
    }, 150); logMessage(`[ServerStatus] Details ${showDetails ? 'hidden' : 'shown'}, status: ${status}`);
  };
  ...
  const handleSettings = () => {
    ...
    }, 150); logMessage(`[ServerStatus] Settings ${showSettings ? 'hidden' : 'shown'}`);
  };
  ...
  const handleServerUriBlur = () => { // Don't immediately clear editing flag - wait for save or cancel
  };
  ...
  const handleSaveServerConfig = async () => {
    if (!communicationMethods.updateServerConfig || hasBackgroundError) { logMessage('[ServerStatus] Background communication not available');
      return;
    }
    ...
    // Use a single stable message throughout the process to prevent flickers const stableMessage = 'Saving configuration and connecting...';
    setStatusMessage(stableMessage);

    // Clear any existing error setLastErrorMessage('');
    ...
    const minDisplayDuration = 1500; // Minimum 1.5 seconds to prevent jitter
      ...
      await updateServerConfig({ uri: serverUri, connectionType }); logMessage('[ServerStatus] Server config updated successfully');
      ...
      if (success) { setStatusMessage('Successfully connected to MCP server');
      ...
      } else { setStatusMessage('Failed to connect to new MCP server');
      }
  ...
  const getStatusInfo = () => {
    ...
    const baseColors = { emerald: { text: 'text-emerald-500', bg: 'bg-emerald-100', darkBg: 'dark:bg-emerald-900/20' }, amber: { text: 'text-amber-500', bg: 'bg-amber-100', darkBg: 'dark:bg-amber-900/20' }, rose: { text: 'text-rose-500', bg: 'bg-rose-100', darkBg: 'dark:bg-rose-900/20' }, slate: { text: 'text-slate-500', bg: 'bg-slate-100', darkBg: 'dark:bg-slate-900/20' },
    };

    // Determine status display const displayStatus = isReconnecting ? 'reconnecting' : status;
  
      switch (displayStatus) { case 'connected':
        return {
          color: baseColors.emerald.text,
          bgColor: cn(baseColors.emerald.bg, baseColors.emerald.darkBg),
          icon: <Icon name="check" className={baseColors.emerald.text} />, label: 'Connected',
        }; case 'reconnecting':
        return {
          color: baseColors.amber.text,
          bgColor: cn(baseColors.amber.bg, baseColors.amber.darkBg), icon: <Icon name="refresh" className={cn(baseColors.amber.text, 'animate-spin')} />, label: 'Reconnecting',
        }; case 'disconnected':
        return {
          color: baseColors.rose.text,
          bgColor: cn(baseColors.rose.bg, baseColors.rose.darkBg),
          icon: <Icon name="x" className={baseColors.rose.text} />, label: 'Disconnected',
        }; case 'error':
        return {
          color: baseColors.rose.text,
          bgColor: cn(baseColors.rose.bg, baseColors.rose.darkBg),
          icon: <Icon name="info" className={baseColors.rose.text} />, label: 'Error',
        };
      default: // Unknown status
        return {
          color: baseColors.slate.text,
          bgColor: cn(baseColors.slate.bg, baseColors.slate.darkBg),
          icon: <Icon name="info" className={baseColors.slate.text} />, label: 'Unknown',
        };
    }
  };
  ...
  // Determine if we should show enhanced visual cues for disconnected/error states const isDisconnectedOrError = status === 'disconnected' || status === 'error';
  
    return (
    <div
      className={cn( 'relative px-4 py-3 border-b border-slate-200 dark:border-slate-800 transition-all duration-300 ease-out server-status-stable',
      ...
      <div className="relative z-10 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            className={cn( 'relative flex items-center justify-center w-8 h-8 rounded-full transition-all duration-300 ease-out server-status-icon',
              statusInfo.bgColor,
              // Simplified animations to prevent flickers isReconnecting ? 'animate-spin' : isDisconnectedOrError && 'animate-pulse',
            )}>
          ...
          <div className="flex flex-col">
            <Typography
              variant="body"
              className={cn( 'font-semibold transition-colors duration-200 leading-tight',
                // Enhanced text styling with smooth color transitions
                isDisconnectedOrError ? 'text-rose-700 dark:text-rose-400' : status === 'connected' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-200',
              )}>
            ...
            <div
              className={cn( 'text-xs mt-0.5 transition-all duration-200 ease-out max-h-20 overflow-hidden status-message-stable',
                isDisconnectedOrError ? 'text-rose-600 dark:text-rose-400 font-medium' : 'text-slate-500 dark:text-slate-400',
              )}>
              {statusMessage}
            </div>
          </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className={cn( 'group relative p-2 rounded-lg transition-all duration-200 ease-out hover:scale-105 active:scale-95', isReconnecting ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-md',
              // Dynamic button styling based on state
              isDisconnectedOrError ? 'text-rose-600 hover:text-rose-700 hover:bg-rose-100 dark:text-rose-400 dark:hover:text-rose-300 dark:hover:bg-rose-900/30' : 'text-blue-600 hover:text-blue-700 hover:bg-blue-50 dark:text-blue-400 dark:hover:text-blue-300 dark:hover:bg-blue-900/30',
            )}
            ...
            <Icon
              name="refresh"
              size="sm"
              className={cn( 'transition-transform duration-300', isReconnecting ? 'animate-spin' : 'group-hover:rotate-180',
              )}
            />
          </button>
          ...
          <button
            ...
            className={cn( 'group relative p-2 rounded-lg transition-all duration-200 ease-out hover:scale-105 active:scale-95', 'text-slate-500 hover:text-slate-700 hover:bg-slate-100 hover:shadow-md dark:text-slate-400 dark:hover:text-slate-300 dark:hover:bg-slate-800',
            )}
            aria-label="Server settings"
            title="Server settings">
            <Icon
              name="settings"
              size="sm" className={cn('transition-transform duration-200', showSettings ? 'rotate-90' : 'group-hover:rotate-45')}
            />
          </button>
        ...
        <div className="mt-2 p-2 bg-rose-100 dark:bg-rose-900/20 rounded-md border border-rose-200 dark:border-rose-800/50">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1">
              <Typography variant="small" className="text-rose-600 dark:text-rose-400 font-medium"> {status === 'disconnected' ? 'Server connection lost. Click the refresh button to reconnect.' : 'Server connection error. Check your configuration and try again.'}
              </Typography>
          ...
          <Card className="border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg">
            <CardContent className="p-4 text-xs text-slate-700 dark:text-slate-300">
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400"> {connectionType === 'sse'  ? 'HTTP-based streaming connection (traditional)'  : connectionType === 'websocket' ? 'Full-duplex WebSocket connection (faster, more features)' : 'Advanced HTTP streaming (modern MCP protocol)'}
                </p>
              </div>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onBlur={handleServerUriBlur} placeholder={connectionType === 'sse' 
                    ? "http://localhost:3055/sse"  : connectionType === 'websocket'
                ...
                <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <div className="bg-slate-100 dark:bg-slate-800 p-2 rounded font-mono text-xs border"> npx @alsania-io/mcpnyx@latest --config ./config.json --outputTransport {connectionType === 'sse' ? 'sse' : connectionType === 'websocket' ? 'ws' : 'streamableHttp'}
                  </div>
            ...
            <CardContent className="p-4 text-xs text-slate-700 dark:text-slate-300">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center py-1">
                  ...
                  <span
                    className={cn( 'px-2 py-1 rounded-full text-xs font-medium', status === 'connected' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400' : status === 'disconnected' ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/20 dark:text-rose-400' : 'bg-slate-100 text-slate-700 dark:bg-slate-900/20 dark:text-slate-400',
                    )}>
                    {statusInfo.label}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-start py-1">
                  ...
                  <span className="text-right text-slate-600 dark:text-slate-300 max-w-[200px] break-all"> {serverUri || 'Not configured'}
                  </span>
                </div>

                <div className="flex justify-between items-center py-1">
                  <span className="font-medium text-slate-700 dark:text-slate-200">Connection Type:</span>
                  <span className={cn( 'px-2 py-1 rounded-full text-xs font-medium', connectionType === 'websocket'  ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400' : connectionType === 'streamable-http' ? 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-900/20 dark:text-gray-400',
