...
export class QwenAdapter extends BaseAdapterPlugin { readonly name = 'QwenAdapter'; readonly version = '1.0.0'; // Incremented for new architecture readonly hostnames = ['qwen.ai', 'chat.qwen.ai'];
 ...
 // CSS selectors for Qwen's UI elements
  ...
  private readonly selectors = {
    ...
    // Submit button selectors (multiple fallbacks) SUBMIT_BUTTON: '#send-message-button, button._sendMessageButton_71e98_48',
    // File upload related selectors FILE_UPLOAD_BUTTON: 'button[aria-controls*="upload"], input[type="file"]',
    FILE_INPUT: 'input[type="file"][multiple], input#filesUpload',
    // Main panel and container selectors MAIN_PANEL: 'form.flex.w-full.gap-1\.5',
    // Drop zones for file attachment DROP_ZONE: 'textarea#chat-input',
  ...
  async deactivate(): Promise<void> {
    // Guard against double deactivation if (this.currentStatus === 'inactive' || this.currentStatus === 'disabled') { this.context?.logger.warn('Qwen adapter already inactive, skipping deactivation');
      return;
    }

    await super.deactivate(); this.context.logger.debug('Deactivating Qwen adapter...');
  ...
  async cleanup(): Promise<void> {
    await super.cleanup(); this.context.logger.debug('Cleaning up Qwen adapter...');
    ...
    // Remove injected adapter styles const styleElement = document.getElementById('mcp-qwen-button-styles');
    if (styleElement) {
      styleElement.remove();
  ...
  async insertText(text: string, options?: { targetElement?: HTMLElement }): Promise<boolean> {
    this.context.logger.debug( `Attempting to insert text into Qwen chat input: ${text.substring(0, 50)}${text.length > 50 ? '...' : ''}`,
    );
    ...
    if (!targetElement) { this.context.logger.error('Could not find Qwen chat input element'); this.emitExecutionFailed('insertText', 'Chat input element not found');
      return false;
    }

    try {
      // Use textarea value method for Qwen this.context.logger.debug('Using textarea value method for text insertion');
      return await this.insertTextViaTextareaValue(targetElement, text);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error inserting text into Qwen chat input: ${errorMessage}`); this.emitExecutionFailed('insertText', errorMessage);
      return false;
    }
  }
  ...
  private async insertTextViaTextareaValue(element: HTMLElement, text: string): Promise<boolean> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Textarea value method failed: ${errorMessage}`); this.emitExecutionFailed('insertText', `Textarea value method failed: ${errorMessage}`);
      return false;
    }
  }
   ...
   */
  async submitForm(options?: { formElement?: HTMLFormElement }): Promise<boolean> { this.context.logger.debug('Attempting to submit Qwen chat input');
    ...
    if (!submitButton) {
      ...
      if (chatInput) { submitButton = chatInput.closest('form')?.querySelector('button[type="submit"]') as HTMLButtonElement;
    ...
    if (submitButton) {
      try {
        ...
        if (isDisabled) { this.context.logger.warn('Qwen submit button is disabled, waiting for it to be enabled');
        ...
        if (rect.width === 0 || rect.height === 0) { this.context.logger.warn('Qwen submit button is not visible, falling back to Enter key');
          return this.submitWithEnterKey();
        }
 ...
 this.context.logger.debug('Qwen chat input submitted successfully via button click');
    ...
    } else { this.context.logger.warn('Could not find Qwen submit button, falling back to Enter key');
      return this.submitWithEnterKey();
    }
  }
 ...
 this.context.logger.debug('Z chat input submitted successfully via Enter key');
  ...
  async attachFile(file: File, options?: { inputElement?: HTMLInputElement }): Promise<boolean> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error attaching file to Qwen: ${errorMessage}`); this.emitExecutionFailed('attachFile', errorMessage);
      return false;
    }
  }
 ...
 const dropEvent = new DragEvent('drop', {
  ...
  isSupported(): boolean | Promise<boolean> {
    ...
    const isQwenHost = this.hostnames.some(hostname => { if (typeof hostname === 'string') {
   ...
   */
  supportsFileUpload(): boolean { this.context.logger.debug('Checking file upload support for Qwen');
 ...
 this.context.logger.debug('No file upload support detected');
  ...
  private cleanupDOMObservers(): void { this.context.logger.debug('Cleaning up DOM observers for Qwen adapter');
  ...
  private cleanupUIIntegration(): void { this.context.logger.debug('Cleaning up UI integration for Qwen adapter');
  ...
  private handleToolExecutionCompleted(data: any): void { this.context.logger.debug('Handling tool execution completion in Qwen adapter:', data);

    // Use the base class method to check if we should handle events
    if (!this.shouldHandleEvents()) { this.context.logger.debug('Qwen adapter should not handle events, ignoring tool execution event');
      return;
    }
  ...
  private findButtonInsertionPoint(): { container: Element; insertAfter: Element | null; insertBefore?: Element | null } | null { this.context.logger.debug('Finding button insertion point for MCP popover');
    ...
    if (submitButton) {
      ...
      if (container) { this.context.logger.debug('Found submit button container, placing MCP button to its left');
        // Insert before the submit button (to the left)
        return { container, insertAfter: null, insertBefore: submitButton };
      }
    }

    // Fallback 1: Look for the absolute positioned container with submit button const absoluteContainer = document.querySelector('div.flex.items-end.absolute.right-3');
    if (absoluteContainer) { this.context.logger.debug('Found absolute positioned container'); const submitBtn = absoluteContainer.querySelector('#send-message-button');
      if (submitBtn) {
 ...
 // Fallback 3: Look for the main input area's action buttons container
    ...
    if (actionsContainer) { this.context.logger.debug('Found actions container (fallback)');
 ...
 this.context.logger.debug('Could not find suitable insertion point for MCP popover');
  ...
  private injectMCPPopover(insertionPoint: { container: Element; insertAfter: Element | null; insertBefore?: Element | null }): void { this.context.logger.debug('Injecting MCP popover into Qwen interface');
    ...
    try {
      ...
      // Create container for the popover const reactContainer = document.createElement('div'); reactContainer.id = 'mcp-popover-container'; reactContainer.style.display = 'inline-flex'; reactContainer.style.margin = '0 8px 0 0'; // Right margin to create space before submit button
      ...
      if (insertBefore && insertBefore.parentNode === container) {
        ...
        container.insertBefore(reactContainer, insertBefore); this.context.logger.debug('Inserted popover container before specified element (submit button)');
        ...
        .then(React => { import('react-dom/client')
            .then(ReactDOM => { import('../../components/mcpPopover/mcpPopover')
                .then(({ MCPPopover }) => {
                  ...
                  const adapterButtonConfig = { className: 'mcp-qwen-button-base', contentClassName: 'mcp-qwen-button-content', textClassName: 'mcp-qwen-button-text', activeClassName: 'mcp-button-active',
                  };
 ...
 this.context.logger.debug('MCP popover rendered successfully with new architecture');
  ...
  onPageChanged?(url: string, oldUrl?: string): void { this.context.logger.debug(`Qwen page changed: from ${oldUrl || 'N/A'} to ${url}`);
  ...
  onHostChanged?(newHost: string, oldHost?: string): void { this.context.logger.debug(`Qwen host changed: from ${oldHost || 'N/A'} to ${newHost}`);

    // Re-check if the adapter is still supported
    const stillSupported = this.isSupported();
    if (!stillSupported) { this.context.logger.warn('Qwen adapter no longer supported on this host/page');
 ...
 const styleElement = document.createElement('style');
      ...
      this.adapterStylesInjected = true; this.context.logger.debug('Qwen button styles injected successfully');
    } catch (error) {
