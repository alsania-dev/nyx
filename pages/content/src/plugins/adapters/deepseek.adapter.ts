...
export class DeepSeekAdapter extends BaseAdapterPlugin { readonly name = 'DeepSeekAdapter'; readonly version = '2.0.0'; // Incremented for new architecture readonly hostnames = ['chat.deepseek.com'];
 ...
 // CSS selectors for DeepSeek's UI elements
  ...
  private readonly selectors = {
    ...
    // Submit button selectors (multiple fallbacks) SUBMIT_BUTTON: 'button[aria-label*="Send"], button[data-testid="send-button"], button.send-button, svg.send-icon',
    // File upload related selectors FILE_UPLOAD_BUTTON: 'button[aria-label*="attach"], button[aria-label*="file"], input[type="file"]', FILE_INPUT: 'input[type="file"]',
  ...
  async deactivate(): Promise<void> {
    // Guard against double deactivation if (this.currentStatus === 'inactive' || this.currentStatus === 'disabled') { this.context?.logger.warn('DeepSeek adapter already inactive, skipping deactivation');
      return;
    }

    await super.deactivate(); this.context.logger.debug('Deactivating DeepSeek adapter...');
  ...
  async cleanup(): Promise<void> {
    await super.cleanup(); this.context.logger.debug('Cleaning up DeepSeek adapter...');
   ...
   */
  async insertText(text: string, options?: { targetElement?: HTMLElement }): Promise<boolean> { this.context.logger.debug(`Attempting to insert text into DeepSeek chat input: ${text.substring(0, 50)}${text.length > 50 ? '...' : ''}`);
    ...
    if (!targetElement) { this.context.logger.error('Could not find DeepSeek chat input element'); this.emitExecutionFailed('insertText', 'Chat input element not found');
      return false;
    }
    ...
    try {
      ...
      // Handle different input types if (targetElement.tagName === 'TEXTAREA') {
        ...
        // Trigger input event textarea.dispatchEvent(new InputEvent('input', { bubbles: true })); textarea.dispatchEvent(new Event('change', { bubbles: true }));
          ...
          this.context.logger.debug(`Text inserted into textarea successfully. Original: ${currentText.length}, Added: ${text.length}, Total: ${newContent.length}`); } else if (targetElement.getAttribute('contenteditable') === 'true') {
        // Handle contenteditable div const currentText = targetElement.textContent || '';
        ...
        // Insert text with proper formatting if (currentText && currentText.trim() !== '') { document.execCommand('insertText', false, '\n\n');
        } document.execCommand('insertText', false, text);

        // Trigger input event for contenteditable targetElement.dispatchEvent(new InputEvent('input', { bubbles: true })); targetElement.dispatchEvent(new Event('change', { bubbles: true }));
      ...
      this.context.logger.error(`Error inserting text into DeepSeek chat input: ${errorMessage}`); this.emitExecutionFailed('insertText', errorMessage);
      return false;
    }
  }
   ...
   */
  async submitForm(options?: { formElement?: HTMLFormElement }): Promise<boolean> { this.context.logger.debug('Attempting to submit DeepSeek chat input');
    ...
    if (!submitButton) { this.context.logger.warn('Could not find DeepSeek submit button, trying Enter key press');
      return this.tryEnterKeySubmission();
    }

    try {
      // Check if the button is disabled
      if (submitButton.disabled) { this.context.logger.warn('DeepSeek submit button is disabled'); this.emitExecutionFailed('submitForm', 'Submit button is disabled');
        return false;
      }

      // Check if the button is visible and clickable
      const rect = submitButton.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0) { this.context.logger.warn('DeepSeek submit button is not visible'); this.emitExecutionFailed('submitForm', 'Submit button is not visible');
        return false;
      }
 ...
 this.context.logger.debug('DeepSeek chat input submitted successfully');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error submitting DeepSeek chat input: ${errorMessage}`); this.emitExecutionFailed('submitForm', errorMessage);
      return false;
    }
  }
 ...
 this.context.logger.debug('DeepSeek chat input submitted using Enter key');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error submitting DeepSeek chat input via Enter key: ${errorMessage}`); this.emitExecutionFailed('submitForm', errorMessage);
      return false;
    }
  }
 ...
 this.emitExecutionFailed('attachFile', 'All file attachment methods failed');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error attaching file to DeepSeek: ${errorMessage}`); this.emitExecutionFailed('attachFile', errorMessage);
      return false;
    }
  }
 ...
 const dropEvent = new DragEvent('drop', {
  ...
  isSupported(): boolean | Promise<boolean> {
    ...
    const isDeepSeekHost = this.hostnames.some(hostname => { if (typeof hostname === 'string') {
   ...
   */
  supportsFileUpload(): boolean { this.context.logger.debug('Checking file upload support for DeepSeek');
 ...
 this.context.logger.debug('No file upload support detected');
  ...
  private cleanupDOMObservers(): void { this.context.logger.debug('Cleaning up DOM observers for DeepSeek adapter');
  ...
  private cleanupUIIntegration(): void { this.context.logger.debug('Cleaning up UI integration for DeepSeek adapter');
  ...
  private handleToolExecutionCompleted(data: any): void { this.context.logger.debug('Handling tool execution completion in DeepSeek adapter:', data);

    // Use the base class method to check if we should handle events
    if (!this.shouldHandleEvents()) { this.context.logger.debug('DeepSeek adapter should not handle events, ignoring tool execution event');
      return;
    }
  ...
  private findButtonInsertionPoint(): { container: Element; insertAfter: Element | null } | null { this.context.logger.debug('Finding button insertion point for MCP popover');
    ...
    // Try DeepSeek-specific button container first (.ec4f5d61) const buttonContainer = document.querySelector('.ec4f5d61');
    if (buttonContainer) { this.context.logger.debug('Found DeepSeek button container (.ec4f5d61)');

      // Find the attach button container (.bf38813a) - we want to insert BEFORE this const attachContainer = buttonContainer.querySelector('.bf38813a');
      if (attachContainer) { this.context.logger.debug('Found attach button container, will insert before it');
        // Find the last toggle button (Search button) before the attach container const toggleButtons = buttonContainer.querySelectorAll('.ds-toggle-button');
        if (toggleButtons.length > 0) {
          const lastToggleButton = toggleButtons[toggleButtons.length - 1]; this.context.logger.debug('Will insert after last toggle button (Search)');
      ...
      // Fallback: Look for search button specifically const buttons = buttonContainer.querySelectorAll('.ds-button');
      for (const button of Array.from(buttons)) {
        const buttonText = button.textContent?.trim(); if (buttonText === 'Search') { this.context.logger.debug('Found search button, will insert after it');
          return { container: buttonContainer, insertAfter: button };
        }
      }

      // If search button not found, use last button const lastButton = buttonContainer.querySelector('.ds-button:last-child');
      if (lastButton) { this.context.logger.debug('Using last button as insertion point');
        return { container: buttonContainer, insertAfter: lastButton };
      }
    }

    // Try fallback selectors
    const fallbackSelectors = [ '._24fad49', // Textarea parent '.bf38813a', // File upload container '.aaff8b8f', // Chat input area '.chat-input-actions', '.input-actions', '.actions-wrapper'
    ];
 ...
 this.context.logger.debug('Could not find suitable insertion point for MCP popover');
  ...
  private injectMCPPopover(insertionPoint: { container: Element; insertAfter: Element | null }): void { this.context.logger.debug('Injecting MCP popover into DeepSeek interface');
    ...
    try {
      ...
      // Inject DeepSeek-specific button styles if (!document.getElementById('mcp-deepseek-button-styles')) { const styleEl = document.createElement('style'); styleEl.id = 'mcp-deepseek-button-styles';
        styleEl.textContent = this.deepseekButtonStyles;
        document.head.appendChild(styleEl); this.context.logger.debug('DeepSeek button styles injected');
      }

      // Create container for the popover const reactContainer = document.createElement('div'); reactContainer.id = 'mcp-popover-container'; reactContainer.style.display = 'inline-block';
  ...
  private renderMCPPopover(container: HTMLElement): void { this.context.logger.debug('Rendering MCP popover with new architecture integration');
    ...
    try {
      // Import React and ReactDOM dynamically to avoid bundling issues import('react').then(React => { import('react-dom/client').then(ReactDOM => { import('../../components/mcpPopover/mcpPopover').then(({ MCPPopover }) => {
            ...
            const adapterButtonConfig = { className: 'mcp-ds-button', contentClassName: 'mcp-ds-button-content', textClassName: 'mcp-ds-button-text', iconClassName: 'mcp-ds-button-icon', activeClassName: 'mcp-button-active'
            };
            ...
            root.render(
              React.createElement(MCPPopover, {
                ...
                adapterButtonConfig: adapterButtonConfig, adapterName: 'DeepSeekAdapter'
              })
            );
 ...
 context.logger.debug(`MCP toggle completed: MCP ${enabled ? 'enabled' : 'disabled'}, sidebar ${enabled ? 'shown' : 'hidden'}`);
  ...
  onPageChanged?(url: string, oldUrl?: string): void { this.context.logger.debug(`DeepSeek page changed: from ${oldUrl || 'N/A'} to ${url}`);
  ...
  onHostChanged?(newHost: string, oldHost?: string): void { this.context.logger.debug(`DeepSeek host changed: from ${oldHost || 'N/A'} to ${newHost}`);

    // Re-check if the adapter is still supported
    const stillSupported = this.isSupported();
    if (!stillSupported) { this.context.logger.warn('DeepSeek adapter no longer supported on this host/page');
