...
export class OpenRouterAdapter extends BaseAdapterPlugin { readonly name = 'OpenRouterAdapter'; readonly version = '2.0.0'; readonly hostnames = ['openrouter.ai'];
 ...
 // CSS selectors for OpenRouter's UI elements
  private readonly selectors = {
    ...
    SUBMIT_BUTTON: 'button[data-state="closed"] svg[stroke="currentColor"] path[d*="M4.5 10.5"], button[aria-label="Send message"], button[data-testid="send-button"], button[aria-label="Send prompt"]',
    // File upload related selectors - updated for new attachment button
    FILE_UPLOAD_BUTTON: 'button svg[fill="currentColor"] path[fill-rule="evenodd"][d*="M18.97 3.659"], button[aria-label="Attach file"], button[aria-label*="attach"], input[type="file"]', FILE_INPUT: 'input[type="file"]',
    // Main panel and container selectors - updated for new structure
    MAIN_PANEL: '.rounded-xl.overflow-hidden.p-2.border.border-slate-4, .chat-container, .main-content, .conversation-container',
    // Drop zones for file attachment - updated for new structure with proper escaping DROP_ZONE: '.rounded-xl.overflow-hidden.p-2.border.border-slate-4, .rounded-lg.w-full.focus-within\\:bg-accent, textarea[placeholder="Start a new message..."], .chat-input-area, .input-container',
  ...
  async deactivate(): Promise<void> {
    // Guard against double deactivation if (this.currentStatus === 'inactive' || this.currentStatus === 'disabled') { this.context?.logger.warn('OpenRouter adapter already inactive, skipping deactivation');
      return;
    }

    await super.deactivate(); this.context.logger.debug('Deactivating OpenRouter adapter...');
  ...
  async cleanup(): Promise<void> {
    await super.cleanup(); this.context.logger.debug('Cleaning up OpenRouter adapter...');
  ...
  async insertText(text: string, options?: { targetElement?: HTMLElement }): Promise<boolean> {
    ...
    if (!targetElement) { this.context.logger.error('Could not find OpenRouter chat input element'); this.emitExecutionFailed('insertText', 'Chat input element not found');
      return false;
    }
    ...
    try {
      ...
      // Handle different input types if (targetElement.tagName === 'TEXTAREA') {
        ...
        // Trigger input event textarea.dispatchEvent(new InputEvent('input', { bubbles: true })); } else if (targetElement.getAttribute('contenteditable') === 'true') {
        // For contenteditable elements const currentText = targetElement.textContent || '';
        const newContent = currentText ? `${currentText}\n\n${text}` : text;
 ...
 document.execCommand('insertText', false, `\n\n${text}`);
        ...
        // Fallback for other element types const currentText = targetElement.textContent || '';
        const newContent = currentText ? `${currentText}\n\n${text}` : text;
      ...
      // Additional events for better compatibility targetElement.dispatchEvent(new Event('change', { bubbles: true })); targetElement.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true })); targetElement.dispatchEvent(new KeyboardEvent('keyup', { bubbles: true }));
      ...
      this.context.logger.error(`Error inserting text into OpenRouter chat input: ${errorMessage}`); this.emitExecutionFailed('insertText', errorMessage);
      return false;
    }
  }
   ...
   */
  async submitForm(options?: { formElement?: HTMLFormElement }): Promise<boolean> { this.context.logger.debug('Attempting to submit OpenRouter chat input');
    ...
    try {
      ...
      for (const selector of selectors) {
        ...
        if (element) {
          // Handle SVG case where we need to find the parent button if (element.tagName === 'svg' || element.tagName === 'path') { submitButton = element.closest('button') as HTMLButtonElement;
          } else {
      ...
      if (!submitButton) {
        // Look for the send button by its specific characteristics const sendButtons = document.querySelectorAll('button');
        for (let i = 0; i < sendButtons.length; i++) {
          const button = sendButtons[i]; const svg = button.querySelector('svg[stroke="currentColor"]'); const path = svg?.querySelector('path[d*="M4.5 10.5"]');
          if (path && svg) {
            submitButton = button as HTMLButtonElement; this.context.logger.debug('Found submit button by SVG path structure');
            break;
          }
        }
      }
      ...
      if (submitButton) {
        // Check if button is enabled if (submitButton.disabled || submitButton.getAttribute('aria-disabled') === 'true') { this.context.logger.warn('OpenRouter submit button is disabled, trying alternative methods');
        } else {
 ...
 this.context.logger.debug('OpenRouter chat input submitted successfully via button click');
      ...
      // Fallback: Try form submission const chatInput = document.querySelector(this.selectors.CHAT_INPUT.split(', ')[0]) as HTMLElement;
      if (chatInput) { const form = chatInput.closest('form');
        if (form) { const submitEvent = new SubmitEvent('submit', { bubbles: true, cancelable: true });
          form.dispatchEvent(submitEvent);

          this.emitExecutionCompleted( 'submitForm',
            {},
            {
              success: true, method: 'form.submit',
            },
          );
 this.context.logger.debug('OpenRouter chat input submitted successfully via form submission');
      ...
      if (chatInput) {
        ...
        const keyEvents = [ new KeyboardEvent('keydown', { key: 'Enter', code: 'Enter',
            keyCode: 13,
            which: 13,
            bubbles: true,
            cancelable: true,
            ...
            cancelable: true,
            composed: true,
          }), new KeyboardEvent('keyup', { key: 'Enter', code: 'Enter',
            keyCode: 13,
            which: 13,
            bubbles: true,
            cancelable: true,
        ...
        this.emitExecutionCompleted( 'submitForm',
          {},
          {
            success: true, method: 'enter-key-simulation',
          },
        );
 ...
 this.emitExecutionFailed('submitForm', 'No submission method available');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error submitting OpenRouter chat input: ${errorMessage}`); this.emitExecutionFailed('submitForm', errorMessage);
      return false;
    }
  }
 ...
 this.emitExecutionFailed('attachFile', 'All attachment methods failed');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error attaching file to OpenRouter: ${errorMessage}`); this.emitExecutionFailed('attachFile', errorMessage);
      return false;
    }
  }
  ...
  private findAttachmentButton(): HTMLButtonElement | null {
    // Look for the attachment button using the new structure const buttons = document.querySelectorAll('button');
    for (let i = 0; i < buttons.length; i++) {
      const button = buttons[i]; const svg = button.querySelector('svg[fill="currentColor"]'); const path = svg?.querySelector('path[fill-rule="evenodd"][d*="M18.97 3.659"]');
      if (path && svg) { this.context.logger.debug('Found attachment button by SVG path structure');
        return button as HTMLButtonElement;
      }
    }

    // Fallback selectors
    const fallbackSelectors = [ 'button[aria-label="Attach file"]', 'button[aria-label*="attach"]', 'input[type="file"]',
    ];

    for (const selector of fallbackSelectors) {
      const element = document.querySelector(selector);
      if (element) { if (element.tagName === 'INPUT') { // If it's an input, look for a related button
 ...
 this.context.logger.debug('No attachment button found');
  ...
  private async tryCopyPasteAttachment(file: File): Promise<boolean> {
    try {
      ...
      // Find the chat input textarea const chatInput = document.querySelector('textarea[placeholder="Start a new message..."]') as HTMLTextAreaElement;
      if (!chatInput) { this.context.logger.debug('Chat input textarea not found');
        return false;
      }
      ...
      // Method 1: Direct paste event with file in clipboard data this.context.logger.debug('Method 1: Direct paste event simulation');
      const success1 = await this.tryDirectPasteEvent(file, chatInput);
      if (success1) return true;

      // Method 2: Keyboard shortcut simulation this.context.logger.debug('Method 2: Keyboard shortcut simulation');
      const success2 = await this.tryKeyboardPasteSimulation(file, chatInput);
      if (success2) return true;

      // Method 3: Clipboard API approach this.context.logger.debug('Method 3: Clipboard API approach');
      const success3 = await this.tryClipboardAPIApproach(file, chatInput);
      if (success3) return true;

      // Method 4: Firefox-specific input event simulation this.context.logger.debug('Method 4: Firefox-specific input event simulation');
 ...
 this.context.logger.debug('All copy-paste methods failed');
  ...
  private async tryDirectPasteEvent(file: File, chatInput: HTMLTextAreaElement): Promise<boolean> {
    try {
      ...
      // Create paste event with enhanced properties for Firefox const pasteEvent = new ClipboardEvent('paste', {
        bubbles: true,
        cancelable: true,
      ...
      // Firefox-specific: Add additional properties to the event Object.defineProperty(pasteEvent, 'clipboardData', {
        value: clipboardData,
        writable: false,
        configurable: true,
      });
      ...
      const targets = [
        chatInput,
        chatInput.parentElement, chatInput.closest('.rounded-lg.w-full'), chatInput.closest('.rounded-xl.overflow-hidden.p-2.border.border-slate-4'),
        document.activeElement,
      ].filter(Boolean);

      for (const target of targets) {
        if (target) { this.context.logger.debug(`Dispatching paste event on: ${target.tagName}.${target.className || 'no-class'}`);
          target.dispatchEvent(pasteEvent);
          await new Promise(resolve => setTimeout(resolve, 200));
          
          // Check immediately after each dispatch
          const previewFound = await this.checkFilePreview();
          if (previewFound) { this.context.logger.debug('Direct paste event succeeded'); this.emitExecutionCompleted('attachFile', {
              fileName: file.name,
              fileType: file.type,
              fileSize: file.size,
            }, {
              success: true,
              previewFound: true, method: 'direct-paste-event',
            });
            return true;
          }
        }
  ...
  private async tryKeyboardPasteSimulation(file: File, chatInput: HTMLTextAreaElement): Promise<boolean> {
    try {
      ...
      const createKeyEvent = (type: string) => {
        const event = new KeyboardEvent(type, { key: 'v', code: 'KeyV',
          keyCode: 86,
          which: 86,
          ctrlKey: true,
          bubbles: true,
        ...
        // Firefox-specific properties Object.defineProperty(event, 'ctrlKey', { value: true, writable: false }); Object.defineProperty(event, 'metaKey', { value: false, writable: false });
          
          return event;
      };

      // Simulate the keyboard sequence more realistically const keydownEvent = createKeyEvent('keydown'); const keypressEvent = createKeyEvent('keypress'); const keyupEvent = createKeyEvent('keyup');
      ...
      for (const target of targets) {
        if (target) {
          ...
          // Follow up with a paste event that might be triggered by the keyboard event const pasteEvent = new ClipboardEvent('paste', {
            bubbles: true,
            cancelable: true,
            clipboardData: clipboardData,
            composed: true,
          ...
          // Firefox-specific: Ensure clipboardData is properly attached Object.defineProperty(pasteEvent, 'clipboardData', {
            value: clipboardData,
            writable: false,
            configurable: true,
          });
          ...
          const previewFound = await this.checkFilePreview();
          if (previewFound) { this.context.logger.debug('Keyboard paste simulation succeeded');
            ...
            delete (window as any).__mcpClipboardData;
             this.emitExecutionCompleted('attachFile', {
              ...
              success: true,
              previewFound: true, method: 'keyboard-paste-simulation',
            });
            return true;
          }
        }
  ...
  private async tryClipboardAPIApproach(file: File, chatInput: HTMLTextAreaElement): Promise<boolean> {
    try {
      ...
      if (!navigator.clipboard || !navigator.clipboard.write) { this.context.logger.debug('Clipboard API not available, skipping');
        return false;
      }
 this.context.logger.debug('Using Clipboard API to write file');
      ...
      const mimeTypeAttempts = [
        file.type, // Original MIME type 'application/octet-stream', // Generic binary 'text/plain', // Fallback for text files
      ];
 ...
 const pasteEvent = new ClipboardEvent('paste', {
          ...
          const targets = [
            chatInput,
            chatInput.parentElement, chatInput.closest('.rounded-xl.overflow-hidden.p-2.border.border-slate-4'),
            document.activeElement,
          ].filter(Boolean);
          ...
          for (const target of targets) {
            if (target) {
              ...
              const previewFound = await this.checkFilePreview();
              if (previewFound) {
                this.context.logger.debug(`Clipboard API approach succeeded with MIME type: ${mimeType}`); this.emitExecutionCompleted('attachFile', {
                  fileName: file.name,
                  fileType: file.type,
                  fileSize: file.size,
                }, {
                  success: true,
                  previewFound: true, method: 'clipboard-api',
                });
                return true;
              }
            }
  ...
  private async tryFirefoxInputSimulation(file: File, chatInput: HTMLTextAreaElement): Promise<boolean> {
    try { this.context.logger.debug('Attempting Firefox-specific input simulation');

      // Create a DataTransfer with the file
      const dataTransfer = new DataTransfer();
      dataTransfer.items.add(file);

      // Create an input event with file data const inputEvent = new InputEvent('input', {
        bubbles: true,
        cancelable: true,
        composed: true,
        data: null, inputType: 'insertFromPaste',
      });

      // Firefox-specific: Add dataTransfer to the input event Object.defineProperty(inputEvent, 'dataTransfer', {
        value: dataTransfer,
        writable: false,
        configurable: true,
      });

      // Also create a beforeinput event (Firefox sometimes needs this) const beforeInputEvent = new InputEvent('beforeinput', {
        bubbles: true,
        cancelable: true,
        composed: true,
        data: null, inputType: 'insertFromPaste',
 ...
 Object.defineProperty(beforeInputEvent, 'dataTransfer', {
      ...
      for (const target of targets) {
        if (target) { this.context.logger.debug(`Trying Firefox input simulation on: ${target.tagName}.${target.className || 'no-class'}`);
          ...
          // Follow up with a paste event const pasteEvent = new ClipboardEvent('paste', {
            bubbles: true,
            cancelable: true,
 ...
 Object.defineProperty(pasteEvent, 'clipboardData', {
          ...
          // Check for file preview
          const previewFound = await this.checkFilePreview();
          if (previewFound) { this.context.logger.debug('Firefox input simulation succeeded'); this.emitExecutionCompleted('attachFile', {
              fileName: file.name,
              fileType: file.type,
              fileSize: file.size,
            }, {
              success: true,
              previewFound: true, method: 'firefox-input-simulation',
            });
            return true;
          }
        }
  ...
  isSupported(): boolean | Promise<boolean> {
    ...
    // // Check hostname
    // const isOpenRouterHost = this.hostnames.some(hostname => { //   if (typeof hostname === 'string') {
   ...
   */
  supportsFileUpload(): boolean { this.context.logger.debug('Checking file upload support for OpenRouter');

    // Check for the new attachment button structure
    const attachmentButton = this.findAttachmentButton();
    if (attachmentButton) { this.context.logger.debug('Found attachment button - file upload supported');
      return true;
    }
    ...
    // Check for file upload buttons or inputs const uploadSelectors = [...this.selectors.FILE_UPLOAD_BUTTON.split(', '), this.selectors.FILE_INPUT];
 ...
 // Check if we're on a chat page (which should support file upload)
    const currentUrl = window.location.href;
    if (currentUrl.includes('/chat')) { this.context.logger.debug('On chat page - assuming file upload is supported');
      return true;
    }
 this.context.logger.debug('No file upload support detected');
  ...
  private setupDOMObservers(): void {
    ...
    this.mutationObserver = new MutationObserver(mutations => {
      ...
      if (shouldReinject) { // Only attempt re-injection if we're on a chat page
        const currentUrl = window.location.href;
 ...
 // Only attempt re-injection if we're on a chat page
  ...
  private cleanupDOMObservers(): void { this.context.logger.debug('Cleaning up DOM observers for OpenRouter adapter');
  ...
  private cleanupUIIntegration(): void { this.context.logger.debug('Cleaning up UI integration for OpenRouter adapter');
  ...
  private handleToolExecutionCompleted(data: any): void { this.context.logger.debug('Handling tool execution completion in OpenRouter adapter:', data);

    // Use the base class method to check if we should handle events
    if (!this.shouldHandleEvents()) { this.context.logger.debug('OpenRouter adapter should not handle events, ignoring tool execution event');
      return;
    }
  ...
  private findButtonInsertionPoint(): { container: Element; insertAfter: Element | null } | null { this.context.logger.debug('Finding button insertion point for MCP popover');
    ...
    // Try primary selector first - using the configured selectors const primarySelectors = this.selectors.BUTTON_INSERTION_CONTAINER.split(', ');
      ...
      if (wrapper) {
        ...
        // Find the web search container by looking for the text content const flexElements = wrapper.querySelectorAll('div.inline-flex.items-center');
        for (let i = 0; i < flexElements.length; i++) {
          const element = flexElements[i]; if (element.textContent?.includes('Web search')) {
        ...
        if (!webSearchContainer) {
          webSearchContainer =
            wrapper .querySelector('div.inline-flex.items-center svg[stroke="currentColor"] path[d*="M12 21a9.004"]') ?.closest('div.inline-flex.items-center') || null;
        }
    ...
    // Try fallback selectors from the configuration const fallbackSelectors = this.selectors.FALLBACK_INSERTION.split(', ');
 ...
 this.context.logger.debug('Could not find suitable insertion point for MCP popover');
  ...
  private injectMCPPopover(insertionPoint: { container: Element; insertAfter: Element | null }): void { this.context.logger.debug('Injecting MCP popover into OpenRouter interface');
 ...
 const popoverContainer = document.getElementById('mcp-popover-container');
  ...
  private async checkFilePreview(): Promise<boolean> {
    return new Promise(resolve => {
      setTimeout(() => {
        // Check for the specific OpenRouter file preview structure const chatContainer = document.querySelector('.rounded-xl.overflow-hidden.p-2.border.border-slate-4');
        if (chatContainer) {
          // Look for the file preview area that appears after file drop const filePreviewArea = chatContainer.querySelector('.duration-200.bg-accent\\/80.flex.w-full.shadow-inner.p-2');
          if (filePreviewArea) { this.context.logger.debug('Found OpenRouter file preview area');
            resolve(true);
            return;
          }

          // Look for file attachment containers const fileAttachments = chatContainer.querySelectorAll('.bg-background.relative.h-32.w-48');
          ...
          // Look for file preview with filename const fileNameElements = chatContainer.querySelectorAll('.w-40.truncate.pl-2');
          ...
          // Look for the scroll area that contains file previews const scrollArea = chatContainer.querySelector('[data-radix-scroll-area-viewport]');
          if (scrollArea && scrollArea.children.length > 0) { this.context.logger.debug('Found content in scroll area - likely file preview');
            resolve(true);
            return;
          }
        }
        ...
        const previewSelectors = [
          this.selectors.FILE_PREVIEW, '.file-attachment', '.attachment-preview', '.file-upload-preview', '[data-testid*="file"]', '[data-testid*="attachment"]', '.uploaded-file', '.file-item',
          // Check for any elements that might indicate a file was added '[class*="file"]', '[class*="attachment"]', '[class*="upload"]',
          // OpenRouter specific selectors '.group.relative.flex.shrink-0', '.bg-background.relative.h-32',
        ];
        ...
        for (const selector of previewSelectors) {
          ...
          for (let i = 0; i < elements.length; i++) {
            ...
            if (
              ...
              element.offsetWidth > 0 && (element.textContent?.trim() || element.querySelector('img, svg, pre'))
            ) {
 ...
 this.context.logger.debug('No file preview found');
  ...
  // Event handlers
  onPageChanged?(url: string, oldUrl?: string): void { this.context.logger.debug(`OpenRouter page changed: from ${oldUrl || 'N/A'} to ${url}`);
 ...
 this.context.eventBus.emit('app:site-changed', {
  ...
  onHostChanged?(newHost: string, oldHost?: string): void { this.context.logger.debug(`OpenRouter host changed: from ${oldHost || 'N/A'} to ${newHost}`);

    const stillSupported = this.isSupported();
    if (!stillSupported) { this.context.logger.warn('OpenRouter adapter no longer supported on this host/page'); this.context.eventBus.emit('adapter:deactivated', {
        pluginName: this.name,
        timestamp: Date.now(),
      });
    } else {
