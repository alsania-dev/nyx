...
export class KimiAdapter extends BaseAdapterPlugin { readonly name = 'KimiAdapter'; readonly version = '2.0.0'; // Incremented for new architecture readonly hostnames = ['kimi.com'];
 ...
 // CSS selectors for Kimi's UI elements
  ...
  private readonly selectors = {
    ...
    // Submit button selectors (multiple fallbacks) SUBMIT_BUTTON: '.send-button, .send-icon, svg.send-icon, button[aria-label*="Send"], .send-button-container:not(.disabled) .send-button',
    // File upload related selectors FILE_UPLOAD_BUTTON: '.attachment-button, .attachment-icon, input[type="file"], label.attachment-button', FILE_INPUT: 'input[type="file"]',
  ...
  async deactivate(): Promise<void> {
    // Guard against double deactivation if (this.currentStatus === 'inactive' || this.currentStatus === 'disabled') { this.context?.logger.warn('Kimi adapter already inactive, skipping deactivation');
      return;
    }

    await super.deactivate(); this.context.logger.debug('Deactivating Kimi adapter...');
  ...
  async cleanup(): Promise<void> {
    await super.cleanup(); this.context.logger.debug('Cleaning up Kimi adapter...');
   ...
   */
  async insertText(text: string, options?: { targetElement?: HTMLElement }): Promise<boolean> { this.context.logger.debug(`Attempting to insert text into Kimi chat input: ${text.substring(0, 50)}${text.length > 50 ? '...' : ''}`);
    ...
    if (!targetElement) { this.context.logger.error('Could not find Kimi chat input element'); this.emitExecutionFailed('insertText', 'Chat input element not found');
      return false;
    }
    ...
    try {
      ...
      // Handle different input types if (targetElement.tagName === 'TEXTAREA') {
        ...
        // Trigger input event textarea.dispatchEvent(new InputEvent('input', { bubbles: true })); textarea.dispatchEvent(new Event('change', { bubbles: true }));
  
          this.context.logger.debug(`Text inserted into textarea successfully. Original: ${currentText.length}, Added: ${text.length}, Total: ${newContent.length}`); } else if (targetElement.getAttribute('contenteditable') === 'true') { // Handle contenteditable div - Kimi's Lexical editor requires special handling
        ...
        try {
          // Firefox has stricter clipboard security, check for proper permissions const isFirefox = navigator.userAgent.toLowerCase().includes('firefox');
       ...
       this.context.logger.warn('Firefox clipboard read test failed, trying write anyway:', e);
            ...
            if (hasClipboardAccess) {
              // Save current clipboard content (skip in Firefox if it fails) let originalClipboard = '';
              try {
                if (!isFirefox) {
       ...
       this.context.logger.warn('Could not read clipboard, continuing without backup:', e);
              ...
              const clipboardData = new DataTransfer(); clipboardData.setData('text/plain', text); clipboardData.setData('text/html', text.replace(/\n/g, '<br>'));
 const pasteEvent = new ClipboardEvent('paste', {
              ...
              // For Firefox, also try input event with data const inputEvent = new InputEvent('input', {
                bubbles: true,
                cancelable: true, inputType: 'insertFromPaste',
                data: text
              });
              ...
              if (isFirefox) { targetElement.dispatchEvent(new Event('input', { bubbles: true })); targetElement.dispatchEvent(new Event('change', { bubbles: true }));
              }
       ...
       this.context.logger.warn('Could not restore clipboard:', e);
              ...
              insertionSuccess = true; this.context.logger.debug(`Text inserted using clipboard simulation method (${isFirefox ? 'Firefox' : 'Chrome'} mode)`);
            } else { throw new Error('Clipboard access denied');
            }
       ...
       this.context.logger.warn('Clipboard method failed, trying Firefox-specific method:', clipboardError);
          ...
          // Method 1.5: Firefox-specific clipboard workaround using hidden textarea const isFirefox = navigator.userAgent.toLowerCase().includes('firefox');
          if (isFirefox) {
            try {
              // Create a hidden textarea to simulate clipboard operation const hiddenTextarea = document.createElement('textarea');
              hiddenTextarea.value = text; hiddenTextarea.style.position = 'fixed'; hiddenTextarea.style.left = '-9999px'; hiddenTextarea.style.top = '-9999px'; hiddenTextarea.style.opacity = '0'; hiddenTextarea.style.pointerEvents = 'none';
              
              document.body.appendChild(hiddenTextarea);
              hiddenTextarea.select();
              hiddenTextarea.setSelectionRange(0, text.length);
              
              // Try to copy to clipboard using execCommand const copySuccess = document.execCommand('copy');
                
                if (copySuccess) {
                // Now try to paste into the target element
                targetElement.focus(); const pasteSuccess = document.execCommand('paste');
                
                if (pasteSuccess) {
                  insertionSuccess = true; this.context.logger.debug('Text inserted using Firefox hidden textarea method');
                } else {
                  ...
                  insertionSuccess = true; this.context.logger.debug('Text inserted using Firefox execCommand fallback');
                }
              }
              ...
              if (insertionSuccess) {
                // Firefox needs these additional events targetElement.dispatchEvent(new Event('input', { bubbles: true })); targetElement.dispatchEvent(new Event('change', { bubbles: true })); targetElement.dispatchEvent(new InputEvent('input', { 
       ...
       this.context.logger.warn('Firefox-specific method failed:', firefoxError);
          ...
          try {
            ...
            // Split text by lines and insert with proper line breaks const lines = text.split('\n');
            lines.forEach((line, index) => {
              if (index > 0) {
                // Insert line break for subsequent lines document.execCommand('insertLineBreak', false);
       ...
       this.context.logger.warn('execCommand method failed, trying DOM manipulation:', execError);
            ...
            try { let newContent = ''; if (currentText && currentText.trim() !== '') { newContent = currentText + '\n\n' + text;
              } else {
                newContent = text;
              }

              // Clear existing content targetElement.innerHTML = '';

              // Split text by newlines and create proper HTML structure const lines = newContent.split('\n');
              const fragment = document.createDocumentFragment();

              lines.forEach((line, index) => {
                if (index > 0) {
                  // Add line break for subsequent lines fragment.appendChild(document.createElement('br'));
                }
                 if (line.trim() !== '') {
                  // Create text node for non-empty lines
                  fragment.appendChild(document.createTextNode(line));
                } else if (index < lines.length - 1) {
                  // Add empty text node for empty lines to maintain structure fragment.appendChild(document.createTextNode(''));
                }
              });
       ...
       this.context.logger.error('DOM manipulation method also failed:', domError);
        ...
        if (insertionSuccess) {
          // Trigger input events for all methods targetElement.dispatchEvent(new InputEvent('input', { bubbles: true })); targetElement.dispatchEvent(new Event('change', { bubbles: true }));
          
          // Additional events that Lexical might be listening for targetElement.dispatchEvent(new Event('paste', { bubbles: true })); targetElement.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true })); targetElement.dispatchEvent(new KeyboardEvent('keyup', { key: 'Enter', bubbles: true }));
  
            this.context.logger.debug(`Text inserted into Kimi Lexical editor with preserved formatting`);
        } else { throw new Error('All insertion methods failed');
        }
      } else {
        // Fallback for other element types const originalValue = (targetElement as any).value || targetElement.textContent || ''; const newContent = originalValue ? originalValue + '\n\n' + text : text;
      ...
      this.context.logger.error(`Error inserting text into Kimi chat input: ${errorMessage}`); this.emitExecutionFailed('insertText', errorMessage);
      return false;
    }
  }
   ...
   */
  async submitForm(options?: { formElement?: HTMLFormElement }): Promise<boolean> { this.context.logger.debug('Attempting to submit Kimi chat input');
    ...
    if (!submitButton) { this.context.logger.warn('Could not find Kimi submit button, trying Enter key press');
      return this.tryEnterKeySubmission();
    }

    try {
      // Check if the button is disabled
      if (submitButton.disabled) { this.context.logger.warn('Kimi submit button is disabled'); this.emitExecutionFailed('submitForm', 'Submit button is disabled');
        return false;
      }

      // Check if the button is visible and clickable
      const rect = submitButton.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0) { this.context.logger.warn('Kimi submit button is not visible'); this.emitExecutionFailed('submitForm', 'Submit button is not visible');
        return false;
      }
 ...
 this.context.logger.debug('Kimi chat input submitted successfully');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error submitting Kimi chat input: ${errorMessage}`); this.emitExecutionFailed('submitForm', errorMessage);
      return false;
    }
  }
  ...
  private async tryEnterKeySubmission(): Promise<boolean> {
    try {
      // Find the chat input element const chatInput = document.querySelector(this.selectors.CHAT_INPUT.split(', ')[0].trim()) as HTMLElement;
        
        if (!chatInput) { this.context.logger.error('Cannot find chat input for Enter key submission'); this.emitExecutionFailed('submitForm', 'Chat input not found for Enter key submission');
        return false;
      }
      ...
      // Emit success event this.emitExecutionCompleted('submitForm', {}, {
 ...
 this.context.logger.debug('Kimi chat input submitted using Enter key');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error submitting Kimi chat input via Enter key: ${errorMessage}`); this.emitExecutionFailed('submitForm', errorMessage);
      return false;
    }
  }
 ...
 this.emitExecutionFailed('attachFile', 'All file attachment methods failed');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.context.logger.error(`Error attaching file to Kimi: ${errorMessage}`); this.emitExecutionFailed('attachFile', errorMessage);
      return false;
    }
  }
  ...
  private async attachFileToInput(file: File, fileInput: HTMLInputElement): Promise<boolean> {
    try {
      ...
      // Trigger change event fileInput.dispatchEvent(new Event('change', { bubbles: true }));
  ...
  private async simulateFileDrop(file: File): Promise<boolean> {
    try {
      // Find drop zone const dropZones = this.selectors.DROP_ZONE.split(', ');
      ...
      if (!dropZone) { this.context.logger.warn('No drop zone found for file drop simulation');
        return false;
      }
 ...
 const dropEvent = new DragEvent('drop', {
  ...
  isSupported(): boolean | Promise<boolean> {
    ...
    const isKimiHost = this.hostnames.some(hostname => { if (typeof hostname === 'string') {
   ...
   */
  supportsFileUpload(): boolean { this.context.logger.debug('Checking file upload support for Kimi');
 ...
 this.context.logger.debug('No file upload support detected');
  ...
  private cleanupDOMObservers(): void { this.context.logger.debug('Cleaning up DOM observers for Kimi adapter');
  ...
  private cleanupUIIntegration(): void { this.context.logger.debug('Cleaning up UI integration for Kimi adapter');
  ...
  private handleToolExecutionCompleted(data: any): void { this.context.logger.debug('Handling tool execution completion in Kimi adapter:', data);

    // Use the base class method to check if we should handle events
    if (!this.shouldHandleEvents()) { this.context.logger.debug('Kimi adapter should not handle events, ignoring tool execution event');
      return;
    }
  ...
  private findButtonInsertionPoint(): { container: Element; insertAfter: Element | null } | null { this.context.logger.debug('Finding button insertion point for MCP popover');
    ...
    // Try Kimi-specific button container first (.right-area) const buttonContainer = document.querySelector('.chat-editor-action .right-area');
    if (buttonContainer) { this.context.logger.debug('Found Kimi button container (.right-area)');
      
      // Look for attachment button specifically const attachmentButton = buttonContainer.querySelector('.attachment-button');
      if (attachmentButton) { this.context.logger.debug('Found attachment button, will insert after it');
        return { container: buttonContainer, insertAfter: attachmentButton };
      }

      // Look for prompt library button const promptLibraryButton = buttonContainer.querySelector('.prompt-library-button');
      if (promptLibraryButton) { this.context.logger.debug('Found prompt library button, will insert after it');
      ...
      if (lastChild) { this.context.logger.debug('Using last child element as insertion point');
        return { container: buttonContainer, insertAfter: lastChild };
      }
    }

    // Try fallback selectors for Kimi
    const fallbackSelectors = [ '.chat-editor-action', // Main action container '.right-area', // Right area container '.chat-editor', // Chat editor container '.chat-input-editor-container', // Input container '.chat-input-actions', '.input-actions', '.actions-wrapper'
    ];
 ...
 this.context.logger.debug('Could not find suitable insertion point for MCP popover');
  ...
  private injectMCPPopover(insertionPoint: { container: Element; insertAfter: Element | null }): void { this.context.logger.debug('Injecting MCP popover into Kimi interface');
 ...
 context.logger.debug(`MCP toggle completed: MCP ${enabled ? 'enabled' : 'disabled'}, sidebar ${enabled ? 'shown' : 'hidden'}`);
  ...
  onPageChanged?(url: string, oldUrl?: string): void { this.context.logger.debug(`Kimi page changed: from ${oldUrl || 'N/A'} to ${url}`);
  ...
  onHostChanged?(newHost: string, oldHost?: string): void { this.context.logger.debug(`Kimi host changed: from ${oldHost || 'N/A'} to ${newHost}`);

    // Re-check if the adapter is still supported
    const stillSupported = this.isSupported();
    if (!stillSupported) { this.context.logger.warn('Kimi adapter no longer supported on this host/page');
