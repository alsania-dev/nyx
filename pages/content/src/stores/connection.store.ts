...
import type { ConnectionStatus, ServerConfig
} from '../types/stores';
import { createLogger
...
const defaultServerConfig: ServerConfig = { uri: 'http: //localhost:3055/sse', // Default from migration guide, should be configurable connectionType: 'sse',
  timeout: 5000, // ms
  retryAttempts: 3,
  retryDelay: 2000, // ms
}; const initialState: Omit<ConnectionState, 'setStatus' | 'setServerConfig' | 'setLastError' | 'incrementAttempts' | 'resetAttempts' | 'setConnected' | 'setDisconnected' | 'startReconnecting' | 'stopReconnecting'> = { status: 'disconnected',
  ...
  devtools(
    (set, get) => ({
      ...
      setStatus: (status: ConnectionStatus) => {
        ...
        logger.debug(`Status changed from ${oldStatus
    } to: ${status
    ...
    { status, error: get().error || undefined
      ...
      setLastError: (error: string | null) => {
        ...
        if (error) { logger.error('[ConnectionStore] Error set:', error); eventBus.emit('connection:error',
      { error: error
      });
    }
  },
    ...
    }`); eventBus.emit('connection:attempt',
    { attempt: newAttempts, maxAttempts: get().serverConfig.retryAttempts
    });
  },
      ...
      setDisconnected: (error?: string) => {
        set(state => ({ status: error ? 'error' : 'disconnected',
    ...
    { status: get().status, error: error || get().error || undefined
    });
  },

      startReconnecting: () => { if (get().status === 'connected') return; // Don't try to reconnect if already connected set({ isReconnecting: true, status: 'reconnecting'
    }); logger.debug('[ConnectionStore] Reconnecting started...'); eventBus.emit('connection:status-changed', { status: 'reconnecting'
    });
  },

      stopReconnecting: () => {
    // Only stop if actually reconnecting, and revert to a sensible prior state
        if (get().isReconnecting) { const previousStatus = get().error ? 'error' : 'disconnected';
...
/*
const connect = () => {
  const { serverConfig, incrementAttempts, setConnected, setLastError, setStatus } = useConnectionStore.getState(); setStatus('connecting');
  incrementAttempts();
  ...
  fetch(serverConfig.uri)
    ...
    .catch(err => {
      ...
      setLastError(errorMsg); setStatus('error');
