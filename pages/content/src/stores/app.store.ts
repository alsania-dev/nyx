...
// Placeholder for initializePluginRegistry - will be properly imported when plugin system is built const logger = createLogger('initializePluginRegistry'); const initializePluginRegistry = async () => logger.debug('Plugin registry initialized (placeholder)');
...
export const useAppStore = create<AppState>()(
  devtools(
    persist(
      ...
      {
        name: 'nyx-app-store', // Unique name for localStorage
...
if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    // Keep the channel open for asynchronous sendResponse, if needed by other listeners. // For this specific case, it's not strictly necessary to return true unless other parts of the listener use sendResponse.
    return false;
  });
}
