...
// Hook for current adapter operations const logger = createLogger('UseAdapter');
...
export function useAdapterStatus() {
  ...
  useEventListener('adapter:activated', data => {
    setAdapterEvents(prev => [
      ...prev,
      { type: 'activated', adapterName: data.pluginName, timestamp: data.timestamp, data },
    ]);
  });
  useEventListener('adapter:deactivated', data => {
    setAdapterEvents(prev => [
      ...prev,
      { type: 'deactivated', adapterName: data.pluginName, timestamp: data.timestamp, data },
    ]);
  });
